...
def detailed_steps(category: str, title: str, ingredients: list[dict[str, Any]]) -> list[str]:
    ...
    if category == "breakfast":
        ...
        if any(word in lower for word in ("pancake", "waffle", "crepe")):
            return ["Mezcla la harina y el polvo de hornear en un recipiente.", "Bate aparte la leche y el huevo; incorpora la banana triturada cuando la receta la lleve.", "Vierte la mezcla líquida sobre los ingredientes secos y remueve solo hasta que no quede harina visible.", "Deja reposar la masa 5 minutos mientras calientas la superficie de cocción.", "Engrasa ligeramente la sartén y vierte porciones iguales.", f"Cocina cada porción de {title.lower()} de 2 a 3 minutos, hasta que aparezcan burbujas; voltea y cocina de 1 a 2 minutos más, hasta dorar.", "Sirve inmediatamente con la fruta o acompañamiento indicado en los ingredientes."]
