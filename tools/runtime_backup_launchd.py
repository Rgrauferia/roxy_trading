...
DEFAULT_LABEL = "com.roxy.runtime_backup"
DEFAULT_HOUR = 3
DEFAULT_MINUTE = 25
...
def build_backup_args(*, python_path: str | Path, dry_run: bool = False) -> list[str | Path]:
    args: list[str | Path] = [
        Path(python_path),
        TOOLS_DIR / "runtime_backup.py",
    ]
    if dry_run:
        args.append("--dry-run")
    return args
...
def build_shell_command(*, python_path: str | Path, dry_run: bool = False) -> str:
    backup_args = build_backup_args(python_path=python_path, dry_run=dry_run)
    return (
...
def install(args: argparse.Namespace) -> Path:
    ...
    plist = build_plist(
        ...
        hour=args.hour,
        minute=args.minute,
        stdout_path=LAUNCHD_LOG_DIR / "runtime_backup.out",
        stderr_path=LAUNCHD_LOG_DIR / "runtime_backup.err",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Install or control the runtime backup LaunchAgent.")
    ...
    run_now_parser = subparsers.add_parser("run-now", help="Run runtime backup immediately.")
