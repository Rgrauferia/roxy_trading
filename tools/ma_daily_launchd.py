...
DEFAULT_LABEL = "com.roxy.ma_daily"
DEFAULT_HOUR = 18
DEFAULT_MINUTE = 5
...
def build_ma_daily_args(
    *,
    ...
    stock_period: str,
    crypto_limit: int,
    min_buy_hold_edge_pct: float,
    retention_count: int,
    symbols: str | None,
) -> list[str | Path]:
    args: list[str | Path] = [
        Path(python_path),
        TOOLS_DIR / "ma_daily.py",
        "--market",
        market,
        "--limit",
        str(limit),
        ...
        str(min_buy_hold_edge_pct),
        "--retention-count",
        str(retention_count),
    ]
    if refresh_backtests:
        args.append("--refresh-backtests")
    if symbols:
...
def build_shell_command(
    *,
    ...
    min_buy_hold_edge_pct: float = 0.0,
    retention_count: int = 30,
    symbols: str | None = None,
) -> str:
    ma_args = build_ma_daily_args(
        ...
        min_buy_hold_edge_pct=min_buy_hold_edge_pct,
...
def install(args: argparse.Namespace) -> Path:
    ...
    command = build_shell_command(
        ...
        refresh_backtests=args.refresh_backtests,
        stock_period=args.stock_period,
        crypto_limit=args.crypto_limit,
        min_buy_hold_edge_pct=args.min_buy_hold_edge_pct,
    ...
    plist = build_plist(
        ...
        hour=args.hour,
        minute=args.minute,
        stdout_path=LAUNCHD_LOG_DIR / "ma_daily.out",
        stderr_path=LAUNCHD_LOG_DIR / "ma_daily.err",
...
def add_shared_options(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--symbols", help="Comma-separated symbols for targeted daily runs.")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Install or control the daily SMA launchd job.")
    ...
    run_now_parser = subparsers.add_parser("run-now", help="Run the daily workflow immediately.")
