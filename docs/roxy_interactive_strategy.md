...
Wake Roxy can read local voice configuration with "Roxy, estado de voz" / "Roxy, voice status". The brief includes listening mode, speech output, auto-send/dictation mode, selected browser voice, latest local microphone preflight result, default symbol, and watchlist. This is a local diagnostic and does not create a backend assistant turn.
