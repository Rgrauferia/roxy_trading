...
For account or portfolio prompts such as "estado de cuenta", "posiciones abiertas", or "account status", Roxy uses the `account_status` intent. It reads only the local brief/account snapshot, including equity, portfolio value, cash, buying power, open positions, exposure, open P/L, and recent orders when present. If that snapshot is missing, Roxy asks for a paper-broker/account snapshot or explicit account equity instead of inferring balances. This is local account awareness only; it does not call the broker or place orders.
