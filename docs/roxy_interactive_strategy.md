...
For account or portfolio prompts such as "estado de cuenta", "posiciones abiertas", or "account status", Roxy uses the `account_status` intent. It reads only the local brief/account snapshot, including equity, portfolio value, cash, buying power, open positions, exposure, open P/L, recent orders, and largest-position concentration when present. It also labels exposure risk as low, moderate, high, or aggressive from exposure/equity, labels concentration from largest position/equity, and routes high/aggressive exposure or high concentration to risk review before new sizing. If that snapshot is missing, Roxy asks for a paper-broker/account snapshot or explicit account equity instead of inferring balances. This is local account awareness only; it does not call the broker or place orders.

For operational preflight prompts such as "preflight", "revision operativa", "estado operativo", or "operational preflight", Roxy uses the `pre_trade_preflight` intent. It combines local data freshness, market-session timing, account snapshot availability, and the selected/top-ranked opportunity's entry, stop, risk, readiness, and missing confirmations into one voice-ready BLOCKED / WAIT / PREPARE ONLY readout. The preflight is a final local checklist before deeper review; it never approves or executes a trade, and blocked items should route to scan refresh, account snapshot, market-session, checklist, or sizing actions.
...
For headline analysis, Roxy uses the `news_impact` intent. It can read a pasted headline such as `news impact: ...` / `analiza impacto de noticia: ...`, or the first `news` / `market_news` item in the local brief. Local news items preserve source and published timestamp fields when available, and Roxy includes those details in both news lists and impact reads. Parseable timestamps are labeled fresh, aging, or stale by age; stale local news raises priority and routes to refresh/verify the news source before treating it as actionable. If a local item has no timestamp, Roxy labels that gap and routes the user to verify the news time before treating it as fresh. The response classifies a conservative tone as bullish, bearish, or neutral, explains likely market impact, asks the user to verify source and timestamp, and refuses to treat the headline as a standalone trade signal. When the headline or prompt contains a symbol that also has a local opportunity row, Roxy cross-checks the headline against the local action/decision, entry, stop, risk, and missing confirmations, then routes the user to the entry checklist before any trade decision. If no headline or local news item exists, Roxy returns `news_impact_unavailable` with `needs_live_source=true`.
...
Wake Roxy accepts the configured wake word plus common recognition variants for the default name, including `Roxy`, `Roxie`, `Roxy AI`, and `Roxy IA`. Multi-word variants are matched before one-word variants so the assistant removes the full wake phrase before interpreting the command.
...
Roxy Live also exposes `Iniciar voz` and the voice command "Roxy, iniciar voz" / "Roxy, start voice session" as a guided local startup. It first runs the local microphone preflight, then enables browser speech output, auto-send, conversation mode, and Wake Roxy together only when the microphone is not blocked. It selects the compatible feminine browser voice and speaks a short safe-start script, including a low-signal warning when needed. This only prepares the local voice loop; it never executes trades or bypasses explicit confirmation.
...
Wake Roxy can read local voice configuration with "Roxy, estado de voz" / "Roxy, voice status". The brief includes listening mode, speech output, auto-send/dictation mode, selected browser voice, voice quality, latest local microphone preflight result, default symbol, and watchlist. This is a local diagnostic and does not create a backend assistant turn.

Wake Roxy can apply a clear receptionist-style voice preset with "Roxy, voz clara" / "Roxy, receptionist voice", or through the `Voz clara` button. Natural repair phrases such as "Roxy, suena hombre", "Roxy, no te entiendo", or "Roxy, you sound male" trigger the same local preset. The preset reselects the best compatible feminine browser voice, restores Roxy's natural speech rate/pitch, turns browser speech back on, saves those local settings, and reads a short confirmation sample. Roxy Live treats this receptionist preset as the first-load local default and now also runs a voice-readiness guard when the page loads, browser voices change, the language changes, a profile is loaded, listening starts, or speech begins. This prevents a previously saved masculine/heavy or wrong-language browser voice from being reused during live conversation. The selector also labels voice quality, such as `voz femenina clara`, `voz no prioritaria`, or `voz masculina/no recomendada`, and shows the local hint `di: Roxy, voz clara` when the selected voice should be repaired. It is only a browser voice profile change.
...
Roxy Live also exposes `Probar micro` and the voice command "Roxy, probar microfono" / "Roxy, microphone check". This runs a local `getUserMedia` microphone preflight, measures a short Web Audio input-level window, immediately stops the temporary audio stream, and reports ready, quiet, unmeasured, or blocked microphone status. Failures route into the existing blocked microphone recovery path. It does not send audio or text to the assistant backend and does not touch broker or trading flows.
...
Wake Roxy supports natural follow-ups after an answer. "Roxy, mas corto" / "Roxy, shorter" produces a local concise version of the latest reply without recording negative feedback. "Roxy, traducelo al ingles" / "Roxy, say that in Spanish" switches the local language/voice target and prepares a translation follow-up for the latest answer, preserving the same trading safety limits and honoring auto-send or dictation mode. "Roxy, mas detalle", "Roxy, pasos", or "Roxy, explicalo simple" / "Roxy, give more detail", "Roxy, steps", or "Roxy, explain simply" expand into a follow-up prompt about the latest query or answer and then follow the normal auto-send or dictation setting.
