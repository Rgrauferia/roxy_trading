...
El adaptador inicial usa `fal-ai/minimax/hailuo-02/standard/text-to-video`. Los clips duran seis segundos y el costo editable parte de USD 0.045 por segundo, verificado el 21 de agosto de 2026 en la [documentación de fal.ai](https://fal.ai/models/fal-ai/minimax/hailuo-02/standard/text-to-video/api).

El proveedor está aislado detrás de `FalHailuoVideoProvider`; se puede añadir Runway u otro proveedor sin cambiar recetas, usuarios o URLs de reproducción.
...
Con tres clips de seis segundos, la estimación actual es USD 0.81 por receta. La generación permanece deshabilitada si no hay presupuesto, si falta la clave o si el costo estimado supera `ROXY_HOME_VIDEO_MAX_RECIPE_COST_USD`.
