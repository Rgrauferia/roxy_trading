(() => {
  ...
  const validRow = row => row && typeof row.id === 'string' && /^[a-z0-9_-]{1,140}$/i.test(row.id) && typeof row.title === 'string' && row.title.trim() &&
    Object.hasOwn(categories, row.category) && row.category !== 'all' && typeof row.alcoholic === 'boolean' &&
    (row.category !== 'cocktail' || row.alcoholic) && validLines(row.ingredients) && validLines(row.steps) && safeSource(row.source_url) &&
  ...
  function render(container, {user, identity = '', api, hidden = false, active = true, isCurrent = () => true}) {
    ...
    const state = {signature, active:Boolean(active), isCurrent, rows:[], counts:{}, page:1, category:'all', query:'',
      loaded:false, busy:false, adult:false, generation:0, controller:null, utterance:null, speechToken:0, returnFocus:null};
    ...
    const form = node('form', null, 'drinks-search'); const label = node('label'); label.append(node('span', 'Busca una bebida o ingrediente'));
    const search = node('input'); search.type = 'search'; search.maxLength = 100; search.placeholder = 'Café, mango, coffee…'; label.append(search);
    ...
    const closeDialog = (restore = true) => {
      stopSpeech(); if (dialog.open) dialog.close(); dialogInner.replaceChildren();
    ...
    const filtered = () => state.rows.filter(row => (!row.alcoholic || (state.adult && state.category === 'cocktail')) &&
      (state.category === 'all' || row.category === state.category) && normalize([row.title, row.title_es || '', ...row.ingredients, ...(row.ingredients_es || [])].join(' ')).includes(state.query));
    function chooseCategory(key, opener) {
      if (!current()) return;
      if (key !== 'cocktail' || state.adult) { state.category = key; state.page = 1; showPage(true); return; }
      ...
      const confirm = button('Ver cócteles con alcohol', () => {
        if (!current() || !check.checked) return; state.adult = true; state.category = 'cocktail'; state.page = 1; closeDialog(false); showPage(true);
    ...
    function showPage(focus = false) {
      ...
      grid.replaceChildren(...matches.slice((state.page - 1) * 24, state.page * 24).map(row => {
        ...
        const title = node('h3', translated(row) ? row.title_es : row.title); title.lang = translated(row) ? 'es' : 'en';
        const open = button('Preparar bebida', () => { if (current()) showRecipe(row, open); }); open.setAttribute('aria-label', `Preparar bebida: ${title.textContent}`);
        body.append(title, node('p', `${row.ingredients.length} ingredientes · ${row.steps.length} pasos${row.alcoholic ? ' · Con alcohol' : ''}`, 'drinks-card-meta'), open); card.append(body); return card;
      }));
      ...
      status.textContent = matches.length ? `${(state.page - 1) * 24 + 1}–${Math.min(state.page * 24, matches.length)} de ${matches.length} bebidas en esta selección.` : 'No hay bebidas con esos filtros. Prueba otro ingrediente o quita los filtros.';
    ...
    const request = controller => new Promise((resolve, reject) => {
      let settled = false;
      ...
      Promise.resolve().then(() => { if (controller.signal.aborted) throw Object.assign(new Error('cancelled'), {name:'AbortError'}); return api(`/v1/home-food/${encodeURIComponent(user)}/drinks`, {method:'GET', signal:controller.signal}); })
    ...
    async function load() {
      ...
      try {
        ...
        if (!Array.isArray(data.drinks) || data.drinks.length > 60 || !data.drinks.every(validRow) || data.total !== data.drinks.length ||
          new Set(data.drinks.map(row => row.id)).size !== data.drinks.length || !data.counts ||
          data.source?.revision !== revision || data.source?.license !== 'MIT') throw new Error('invalid_catalog');
