(() => {
  ...
  function renderPetCare(pet){const plan=(homeFood.pet_care_plans||{})[String(pet.id)]||{};$('petCareTitle').textContent='Plan de '+pet.name;$('petCareIntro').textContent='Su rutina, alimentación e historial en un solo lugar.';renderPetDailySummary(pet,plan);const root=$('petCarePlan');root.replaceChildren();const shortcuts=document.createElement('div');shortcuts.className='pet-passport-shortcuts';const nutrition=(homeFood.pet_nutrition_plans||{})[String(pet.id)]||{};const medical=[...(pet.medical_history||[])].reverse();const recipeLabel=['fish','reptile','amphibian','invertebrate','farm_pet','other'].includes(pet.species)?'Guías de alimentación':'Recetas seguras';const rows=[['nutrition','nutrition','Plan de alimentación',nutrition.configured?(nutrition.current_food||'Plan confirmado'):'Alimento habitual pendiente de confirmar'],['medical','medical_information','Historial reciente',medical.length?`${medical[0].title} · ${medical[0].occurred_on||'sin fecha'}`:'Añade peso, vacunas o una consulta'],['recipes','menu_book',recipeLabel,'Preparaciones filtradas para su perfil'],['products','shopping_bag','Productos recomendados','Marcas y opciones compatibles para revisar']];rows.forEach(([section,iconName,titleText,detail])=>{const button=document.createElement('button');button.type='button';button.innerHTML=`<span class="material-symbols-rounded" aria-hidden="true">${iconName}</span><span><strong>${escapeHtml(titleText)}</strong><small>${escapeHtml(detail)}</small></span><span class="material-symbols-rounded" aria-hidden="true">chevron_right</span>`;button.addEventListener('click',()=>openPetHubSection(section));shortcuts.append(button)});root.append(shortcuts);const guide=document.createElement('details');guide.className='pet-care-guide';const summary=document.createElement('summary');summary.innerHTML=`<span><strong>Guía personal de ${escapeHtml(pet.name)}</strong><small>Hábitat, convivencia y señales que conviene vigilar</small></span><span class="material-symbols-rounded" aria-hidden="true">expand_more</span>`;guide.append(summary);const content=document.createElement('div');content.className='pet-care-guide-content';(plan.sections||[]).forEach(section=>{const card=document.createElement('article');card.className='pet-care-card'+(section.urgent?' urgent':'')+(section.protected?' protected':'');const icon=document.createElement('span');icon.className='material-symbols-rounded';icon.setAttribute('aria-hidden','true');icon.textContent=section.icon||'pets';const copy=document.createElement('div');const title=document.createElement('h4');title.textContent=section.title;const text=document.createElement('p');text.textContent=section.text;copy.append(title,text);card.append(icon,copy);content.append(card)});if(plan.needs_exact_species){const notice=document.createElement('button');notice.type='button';notice.className='pet-care-exact';notice.innerHTML='<span class="material-symbols-rounded" aria-hidden="true">edit</span><span><strong>Falta la especie exacta</strong><small>Añádela para afinar parámetros, alimentación y convivencia.</small></span>';notice.addEventListener('click',()=>openPetProfile(pet));content.prepend(notice)}if(plan.source_url){const source=document.createElement('a');source.className='pet-care-source';source.href=plan.source_url;source.target='_blank';source.rel='noopener noreferrer';source.textContent='Fuente de cuidado: '+plan.source_label;content.append(source)}if(plan.legal_note){const legal=document.createElement('p');legal.className='pet-care-legal';legal.textContent=plan.legal_note;content.append(legal)}guide.append(content);root.append(guide)}
  ...
  function bind(){
    ...
    $('sharePetVet').addEventListener('click',sharePetWithVet);
    ...
    $('exportPetPassport').addEventListener('click',exportPetMedicalSummary);
