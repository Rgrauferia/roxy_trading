(() => {
  ...
  function renderFamily(){
    ...
    const profileForm=$('familyProfileForm');profileForm.hidden=!viewer;if(viewer){$('familyProfileName').value=viewer.display_name||'';$('familyProfileInitials').textContent=familyInitials(viewer.display_name);const color=viewer.marker_color||'FOREST';profileForm.querySelectorAll('[name="familyMarkerColor"]').forEach(input=>{input.checked=input.value===color});profileForm.querySelector('.family-profile-preview').style.background=familyMarkerColors[color]||familyMarkerColors.FOREST}void renderFamilyMap();
  ...
  async function saveFamilyProfile(event){event.preventDefault();const form=event.currentTarget;const button=form.querySelector('button[type="submit"]');button.disabled=true;try{const marker=form.querySelector('[name="familyMarkerColor"]:checked')?.value||'FOREST';await api('/v1/home-family/profile',{method:'PUT',body:JSON.stringify({display_name:$('familyProfileName').value.trim(),marker_color:marker})});await refreshFamily();announce('Tu identidad de Nexo quedó personalizada')}catch(error){announce(error.message)}finally{button.disabled=false}}
