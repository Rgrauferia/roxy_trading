(() => {
  ...
  let homeFoodReady = false;
  let homeFoodLoadFailed = false;
  ...
  const designPreviewTiers=new Map();
  ...
  const inferShoppingSubcategory = (name,requested='GENERAL') => {
    if(requested==='PETS')return 'Mascotas';
  ...
  const recipeImageLoads=new WeakMap();
  ...
  async function hydrateRecipeImage(image,recipe,host,{hideOnMissing=false,immediate=false}={}){
    ...
    // IntersectionObserver already chose a visible card. Leaving the blob on a
    ...
    for(let attempt=0;attempt<3;attempt+=1){
      ...
      }catch(error){markMissing('No pude cargar la foto. Abre la receta para reintentar.');return}
      finally{clearTimeout(timeout)}
      if(attempt<2)await waitForRecipeImage(3000);
    }
    markMissing()
  }
  ...
  function recipeTranslationBridge(owner,memberId,isCurrent){
    if(account.mode!=='member'||!memberId)return undefined;
    return ({signal,...payload})=>privateRecipeApi(memberId,isCurrent)(`/v1/home-food/${encodeURIComponent(owner)}/recipe-translation`,{
  ...
  function selectPanel(panel,{smooth=true}={}) {
    ...
    if(panel==='family'&&account.mode==='member'){renderFamily();void refreshFamily().catch(()=>{})}
  }
  ...
  function makeShoppingRow(item) {
      ...
      const small=document.createElement('small'); small.textContent=[...new Set([categories[itemCategory]||'Otros',subcategory,item.unit||'unidad'])].filter(Boolean).join(' · ');
  ...
  function recipeCard(recipe){
    ...
    const img=document.createElement('img');img.alt=`Ilustración de ${recipeDisplayTitle(recipe)}`;img.loading='lazy';img.decoding='async';if(recipe.photo_focus)img.style.objectPosition=recipe.photo_focus;hydrateRecipeImage(img,recipe,button);
    const copy=document.createElement('span');const strong=document.createElement('strong');strong.textContent=recipeDisplayTitle(recipe);
    ...
    const servings=Number(recipe.servings||1);const yieldLabel=recipe.audience==='pet'?(recipe.safety_class==='feeding_guide'?'1 guía':'Cantidades para preparar'):`${servings} ${servings===1?'porción':'porciones'}`;const small=document.createElement('small');small.textContent=`${recipe.favorite?'Favorita · ':''}${petLabel||drinkLabel||recipeCategoryLabels[recipeCategoryId(recipe)]||kindLabels[recipe.kind]||'Receta'} · ${yieldLabel} · ${(recipe.steps||[]).length} pasos`;
    const requiresReview=recipe.editorial_status==='needs_canonical_review'||(recipe.audience==='pet'&&recipe.provenance?.can_cook_from_source!==true);
  ...
  const recipeCollectionChoices=[
    ...
    {id:'dessert',title:'Postres',icon:'cake'},
    {id:'local',title:'Mi recetario',icon:'menu_book'},
  ];
  function localRecipeGroup(category){
    if(['coffee_hot','juices','smoothies','cocktails'].includes(category))return 'drinks';
    return category==='desserts'?'dessert':'food';
  }
  function humanRecipeMatchesBrowse(recipe){
    const category=recipeCategoryId(recipe);
    const matchesSearch=!recipeSearch||normalize(`${recipe.title||''} ${recipe.subcategory||''}`).includes(recipeSearch);
    const matchesCategory=recipeFilter==='favorite'?recipe.favorite:recipeFilter==='all'||category===recipeFilter;
    const matchesGroup=['all','favorite'].includes(recipeLocalGroup)||localRecipeGroup(category)===recipeLocalGroup;
    return Boolean(matchesSearch&&matchesCategory&&matchesGroup);
  }
  function chooseRecipeCollection(value){
    if(![...recipeCollectionChoices.map(row=>row.id),'world'].includes(value))return;
    recipeCollection=value;recipeImportExpanded=false;renderRecipes();
    $('recipeCollections').querySelector(`[data-recipe-collection="${value}"]`)?.focus({preventScroll:true});
  }
  function renderRecipeNavigation(petMode){
    ...
    $('recipeWorldButton').setAttribute('aria-pressed',String(recipeCollection==='world'));
  ...
  function renderRecipes() {
    ...
    sourceContext.translate=recipeTranslationBridge(sourceOwner,sourceMember,sourceContext.isCurrent);
    sourceContext.preferredLanguage=recipeProfileEnvelope?.profile?.language||'es';
    ...
    $('recipeHeroBadge').textContent=petMode?'Perfil privado':'Explora a tu ritmo';
    $('recipeImportTitle').textContent=petMode?'Tus mascotas':'Trae cualquier receta a Roxy';
    $('recipeLead').firstChild.textContent=petMode?'Cuidados, alimentación y salud organizados para cada mascota. ':'¿Qué te apetece preparar? Encuentra una receta y consulta su preparación completa. ';
    ...
    if(!petMode)importStudio.classList.remove('pet-onboarding-mode');
    importStudio.hidden=!petMode&&!recipeImportExpanded;
    if(!petMode&&recipeCollection!=='local'){
      onboarding.hidden=true;petHub.hidden=true;catalogSection.hidden=true;
      return;
    }
    $('petLoadingStatus').hidden=!petMode||homeFoodReady;
    $('addPetButton').disabled=petMode&&!homeFoodReady;
    if(!petMode&&!homeFoodReady){
      onboarding.hidden=true;petHub.hidden=true;catalogSection.hidden=false;
    ...
    if(recipeAudience==='pet'){
      renderPetHub();
      if(petHubTab==='care')renderPetHabitat(selectedPetProfile());
    ...
    // Restrictions are screened by whole ingredient terms on the server. A
    ...
    $('recipeCount').textContent=recipeAudience==='pet'?`${rows.length} fichas ${speciesLabel} · ${rows.filter(row=>row.provenance?.can_cook_from_source!==true).length} por verificar`:`${rows.length} ${recipeShowDrafts?'por revisar':rows.length===1?'receta':'recetas'} en esta selección`;
    if(recipeAudience==='pet'){
  ...
  function aquariumInhabitantsField(field,saved=[]){
    ...
    const legend=document.createElement('legend');legend.textContent=field.label;root.append(legend);
    const note=document.createElement('p');note.textContent='Incluye todos los habitantes, también el pez de este perfil. Separa grupos por especie y sexo. Esto no certifica que puedan convivir.';root.append(note);
    const list=document.createElement('div');root.append(list);
    const options=document.createElement('datalist');options.id='aquariumSpeciesOptions';((homeFood.pet_options||{}).exact_species?.fish||[]).forEach(name=>{const option=document.createElement('option');option.value=name;options.append(option)});root.append(options);
    const addRow=(value={})=>{
      if(list.children.length>=12){announce('Máximo 12 grupos por acuario');return}
      const row=document.createElement('fieldset');row.dataset.inhabitant='true';const title=document.createElement('legend');title.textContent='Grupo de habitantes';row.append(title);
      [['species','Especie exacta','text'],['count','Cantidad','number'],['adult_cm','Tamaño adulto (cm)','number']].forEach(([key,text,type])=>{
        const label=document.createElement('label');label.append(document.createTextNode(text));const input=document.createElement('input');input.type=type;input.dataset.field=key;input.value=value[key]??'';
        if(key==='species'){input.required=true;input.maxLength=100;input.setAttribute('list',options.id)}else{input.min=key==='count'?'1':'0.1';input.max=key==='count'?'1000':'500';input.step=key==='count'?'1':'any';input.required=key==='count';input.inputMode='decimal'}label.append(input);row.append(label);
      });
      const label=document.createElement('label');label.append(document.createTextNode('Sexo del grupo'));const select=document.createElement('select');select.dataset.field='sex';[['unknown','No sé'],['male','Machos'],['female','Hembras'],['mixed','Mixto']].forEach(([key,text])=>{const option=document.createElement('option');option.value=key;option.textContent=text;select.append(option)});select.value=value.sex||'unknown';label.append(select);row.append(label);
      const remove=document.createElement('button');remove.type='button';remove.className='secondary';remove.textContent='Quitar grupo';remove.addEventListener('click',()=>row.remove());row.append(remove);list.append(row);
    };
    saved.forEach(addRow);const add=document.createElement('button');add.type='button';add.className='secondary';add.textContent='Añadir habitantes';add.addEventListener('click',()=>addRow());root.append(add);return root;
  }
  function aquariumInhabitantsValues(form){
    return [...form.querySelectorAll('[data-inhabitant]')].map(row=>Object.fromEntries([...row.querySelectorAll('[data-field]')].map(input=>[input.dataset.field,input.value])));
  }
  function habitatValueLabel(value){
    if(Array.isArray(value))return value.map(row=>`${row.count} × ${row.species} · ${({male:'machos',female:'hembras',mixed:'mixto',unknown:'sexo sin confirmar'})[row.sex]||'sexo sin confirmar'}${row.adult_cm?` · adulto ${row.adult_cm} cm`:''}`).join('; ');
    return String(value);
  }
  function renderPetHabitat(pet){
    const root=$('petHabitatPlan');root.replaceChildren();if(!pet)return;
    const plan=(homeFood.pet_habitat_plans||{})[String(pet.id)]||{};
    const guides=(homeFood.pet_care_guides||{})[String(pet.id)]||[];
    const details=document.createElement('details');details.className='pet-habitat-details';
    const summary=document.createElement('summary');summary.textContent=(plan.title||'Cuidados')+' · '+pet.name;details.append(summary);
    const notice=document.createElement('p');notice.className='pet-product-method';notice.textContent=plan.disclosure||'';details.append(notice);
    if(plan.recorded_at){const recorded=document.createElement('p');recorded.textContent='Última observación: '+new Date(plan.recorded_at).toLocaleString('es');details.append(recorded)}
    (plan.alerts||[]).forEach(text=>{const alert=document.createElement('p');alert.className='pet-habitat-alert';alert.textContent=text;details.append(alert)});
    (plan.sections||[]).forEach(section=>{const card=document.createElement('article');card.className='pet-info-card';const copy=document.createElement('div');const title=document.createElement('h4');title.textContent=section.title;const body=document.createElement('p');body.textContent=section.text;copy.append(title,body);card.append(copy);details.append(card)});
    const form=document.createElement('form');form.className='pet-habitat-form';const heading=document.createElement('h4');heading.textContent='Observar y guardar';form.append(heading);
    const help=document.createElement('p');help.textContent='Deja vacío lo que no sepas. Introduce medidas reales en las unidades indicadas; no pondremos valores por ti.';form.append(help);
    (plan.questions||[]).forEach(field=>{
      if(field.kind==='inhabitants'){form.append(aquariumInhabitantsField(field,plan.values?.[field.key]||[]));return}
      const label=document.createElement('label');const caption=document.createElement('span');caption.textContent=field.label+(field.unit?' ('+field.unit+')':'');
      const input=document.createElement(field.kind==='select'?'select':field.kind==='text'?'textarea':'input');input.name=field.key;
      if(field.kind==='select'){const empty=document.createElement('option');empty.value='';empty.textContent='Sin registrar';input.append(empty);(field.options||[]).forEach(value=>{const option=document.createElement('option');option.value=value;option.textContent=value;input.append(option)})}
      else if(field.kind==='number'){input.type='number';input.min=field.min;input.max=field.max;input.step=field.key==='residents'?'1':'any';input.inputMode='decimal'}
      else {input.rows=2;input.maxLength=500}
      input.value=plan.values?.[field.key]??'';label.append(caption,input);form.append(label);
    });
    const status=document.createElement('p');status.setAttribute('role','status');status.setAttribute('aria-live','polite');
    const save=document.createElement('button');save.type='submit';save.className='primary';save.textContent='Guardar observación';form.append(status,save);
    form.addEventListener('submit',async event=>{
      event.preventDefault();if(!form.reportValidity())return;save.disabled=true;status.textContent='Guardando…';
      const values=Object.fromEntries(new FormData(form).entries());
      if(form.querySelector('[data-inhabitants]'))values.inhabitants=aquariumInhabitantsValues(form);
      try{await api(`/v1/home-food/${encodeURIComponent(user)}/pets/${encodeURIComponent(pet.id)}/habitat`,{method:'PUT',body:JSON.stringify({values})});await refreshHomeFood();$('petHabitatPlan').querySelector('details').open=true;announce('Observación guardada para '+pet.name)}
      catch(error){status.textContent=error.message;save.disabled=false}
    });
    if((plan.questions||[]).length)details.append(form);
    if((plan.history||[]).length){const history=document.createElement('details');const label=document.createElement('summary');label.textContent='Observaciones anteriores';history.append(label);const labels=Object.fromEntries((plan.questions||[]).map(field=>[field.key,field]));[...plan.history].reverse().forEach(entry=>{const article=document.createElement('article');const date=document.createElement('h4');date.textContent=new Date(entry.recorded_at).toLocaleString('es');const list=document.createElement('dl');Object.entries(entry.values||{}).filter(([,value])=>value!==null&&value!=='').forEach(([key,value])=>{const term=document.createElement('dt');term.textContent=labels[key]?.label||key;const description=document.createElement('dd');description.textContent=habitatValueLabel(value)+(labels[key]?.unit?' '+labels[key].unit:'');list.append(term,description)});article.append(date,list);history.append(article)});details.append(history)}
    (plan.sources||[]).forEach(source=>{const link=document.createElement('a');link.className='pet-care-source';link.href=source.url;link.target='_blank';link.rel='noopener noreferrer';link.textContent=source.label;details.append(link)});
    root.append(details);
    if(guides.length){const group=document.createElement('details');group.className='pet-habitat-details';const title=document.createElement('summary');title.textContent='Cuidados y alimentación · '+guides.length+' guías';group.append(title);guides.forEach(guide=>{const row=document.createElement('details');const name=document.createElement('summary');name.textContent=guide.title;const intro=document.createElement('p');intro.textContent=guide.description||'';const steps=document.createElement('ol');(guide.steps||[]).forEach(text=>{const step=document.createElement('li');step.textContent=text;steps.append(step)});row.append(name,intro,steps);group.append(row)});root.append(group)}
  }
  function renderPetMedicalSummary(pet){
    const root=$('petMedicalSummary');root.replaceChildren();if(!pet)return;
    const today=new Date();const day=new Date(today.getTime()-today.getTimezoneOffset()*60000).toISOString().slice(0,10);
    const history=pet.medical_history||[];const due=history.filter(row=>row.next_due_on).sort((a,b)=>a.next_due_on.localeCompare(b.next_due_on));
    const status=document.createElement('p');status.className='pet-product-method';status.textContent=history.length?`${history.length} ${history.length===1?'registro conservado':'registros conservados'}. Las fechas provienen de lo que guardaste; Roxy no determina qué vacunas necesita.`:'Sin registros todavía: esto no significa que sus vacunas estén al día.';root.append(status);
    if(due.length){const section=document.createElement('details');const summary=document.createElement('summary');summary.textContent='Fechas de seguimiento guardadas';section.append(summary);due.forEach(row=>{const text=document.createElement('p');text.textContent=`${row.next_due_on} · ${row.title}${row.next_due_on<day?' · Revisa si este control ya se realizó':''}`;section.append(text)});root.append(section)}
    const link=document.createElement('a');link.className='pet-care-source';link.href=pet.species==='bird'?'https://www.aav.org/general/custom.asp?page=FindAVet2':pet.species==='reptile'||pet.species==='amphibian'?'https://arav.org/':'https://www.aaha.org/';link.target='_blank';link.rel='noopener noreferrer';link.textContent=pet.species==='bird'?'Buscar veterinario aviar · AAV':pet.species==='reptile'||pet.species==='amphibian'?'Buscar especialista · ARAV':'Recursos para encontrar atención veterinaria · AAHA';root.append(link);
    const disclosure=document.createElement('small');disclosure.textContent='Directorio externo; no es una afiliación, reserva ni servicio veterinario de Roxy. Confirma que la clínica atienda esta especie.';root.append(disclosure);
  }
  ...
  const petSpeciesLabels={dog:'Perro',cat:'Gato',ferret:'Ferret',rabbit:'Conejo',guinea_pig:'Cobaya',hamster:'Hámster',small_mammal:'Pequeño mamífero',bird:'Ave',fish:'Pez',reptile:'Reptil',amphibian:'Anfibio',invertebrate:'Invertebrado',farm_pet:'Mascota de granja',other:'Otra especie'};
  ...
  function humanRecipeShelf(recipes,showDrafts=false){
    return recipes.filter(recipe=>recipe.audience!=='pet'&&(recipe.editorial_status==='needs_canonical_review')===showDrafts);
  }
  function recipeDisplayTitle(recipe){
    const title=recipe.title||'Receta de Roxy';
    return recipe.pet_species==='ferret'?title.replace(/\bhur[oó]n(?:es)?\b/gi,word=>word.toLowerCase().endsWith('es')?'ferrets':'Ferret'):title;
  }
  function petCapabilities(pet){
    if(!pet)return {recipes:false,recipe_import:false};
    const capability=(homeFood.pet_capabilities||{})[String(pet.id)];
  ...
  function renderPetRecipeAvailability(pet,root){
    const sourceBox=document.createElement('details');sourceBox.className='pet-care-source';sourceBox.setAttribute('aria-label','Fuentes originales de recetas');
    const sourceTitle=document.createElement('summary');sourceTitle.textContent='Consultar originales y referencias';
    const sourceNote=document.createElement('p');sourceNote.textContent='Una guía general de alimentación no valida los ingredientes y pasos de una receta. Las fichas sin original comprobado permanecen visibles, pero no se usan para cocinar ni para añadir ingredientes a Compra.';sourceBox.append(sourceTitle,sourceNote);
    const sourceRows=(homeFood.pet_recipe_sources||{})[String(pet?.id)]||[];
    sourceRows.forEach(source=>{if(!/^https:\/\//.test(source.url||''))return;const link=document.createElement('a');link.href=source.url;link.target='_blank';link.rel='noopener noreferrer';link.textContent=source.title;const note=document.createElement('p');note.textContent=source.note;sourceBox.append(link,note)});
    if(!sourceRows.length){const empty=document.createElement('p');empty.textContent='Todavía no hay una publicación original completa verificada para esta especie. No se sustituye por una receta de perro o gato.';sourceBox.append(empty)}root.append(sourceBox);
  ...
  function renderPetProducts(pet){
    ...
    [['all','Todos'],...categories.map(value=>[value,value])].forEach(([value,label])=>{const button=document.createElement('button');button.type='button';button.className=value===petProductFilter?'active':'';button.dataset.filter=value;button.setAttribute('aria-pressed',String(value===petProductFilter));button.textContent=label;button.addEventListener('click',()=>{const offset=filters.scrollLeft;petProductFilter=value;renderPetProducts(pet);filters.scrollLeft=offset;[...filters.children].find(item=>item.dataset.filter===value)?.focus({preventScroll:true})});filters.append(button)});
    const rows=petProductFilter==='all'?allRows:allRows.filter(row=>row.category===petProductFilter);
    rows.forEach(product=>root.append(makePetProductCard(product,pet)));
  ...
  function makePetProductCard(product,pet){
    const safety=product.safety||{};const blocked=Boolean(safety.cart_blocked);
    const card=document.createElement('article');card.className='pet-product-card';card.append(makePetProductPhoto(product));
    const badge=document.createElement('span');badge.className='pet-product-rank';
    badge.textContent=blocked?'Revisión pendiente':product.identity_specific?'Coincide con su raza o especie':product.personalization_scope==='life_stage'?'Para su etapa':product.personalization_scope==='selected_goal'?'Para su objetivo':'Opción para su especie';
    const copy=document.createElement('div');copy.className='pet-product-copy';
    const brand=document.createElement('small');brand.textContent=product.brand;
    const title=document.createElement('h4');title.textContent=product.name;
    const category=document.createElement('strong');category.textContent=product.category;
    const profile=document.createElement('i');profile.className='pet-product-profile';profile.textContent=product.profile_label||('Para '+pet.name);
    const reasonLabel=document.createElement('b');reasonLabel.textContent=blocked?'Antes de elegir para '+pet.name:'Por qué aparece para '+pet.name;
    const reason=document.createElement('p');reason.textContent=product.reason;
    copy.append(brand,title,category,profile,reasonLabel,reason);
    const reviewArea=document.createElement('div');reviewArea.className='pet-product-review';
    if(safety.message){const notice=document.createElement('p');notice.className='pet-product-safety';notice.textContent=safety.message;reviewArea.append(notice)}
    if(safety.ingredient_review){
      const review=safety.ingredient_review;const detail=document.createElement('details');detail.className='pet-product-ingredients';
      const summary=document.createElement('summary');summary.textContent='Ingredientes y restricciones';
      const highlights=document.createElement('p');highlights.textContent='Entre los ingredientes declarados: '+review.highlights+'. Esto no es una garantía frente a alergias.';
      const link=document.createElement('a');link.href=review.source_url;link.target='_blank';link.rel='noopener noreferrer';link.textContent='Ver fórmula completa del fabricante';
      const date=document.createElement('small');date.textContent='Ficha revisada: '+review.checked_on+'. Comprueba la etiqueta del envase; la fórmula puede cambiar.';
      detail.append(summary,highlights,link,date);reviewArea.append(detail);
    }
    if(product.requires_vet||product.requires_measurement||product.select_before_cart){
      const warning=document.createElement('em');warning.textContent=product.select_before_cart?'Elige primero la fórmula exacta en el catálogo oficial':product.requires_measurement?'Confirma las medidas de tu mascota y de su espacio; Roxy no adivinará la talla ni la capacidad':'Añádelo para revisar; confirma peso, uso o cambio de dieta antes de comprar';reviewArea.append(warning);
    }
    const actions=document.createElement('div');actions.className='pet-product-actions';
    const source=document.createElement('a');source.href=product.source_url;source.target='_blank';source.rel='noopener noreferrer';source.textContent=product.select_before_cart?'Elegir fórmula oficial':'Información oficial';actions.append(source);
    if(!product.select_before_cart&&!blocked){
      const button=makeButton(product.requires_measurement?'Añadir para medir':product.requires_vet?'Añadir para revisar':'Añadir al carrito','primary',async()=>{button.disabled=true;try{await addPetProduct(product,pet)}finally{button.disabled=false}});actions.append(button);
    }
    card.append(badge,copy,reviewArea,actions);return card;
  }
  async function addPetProduct(product,pet){
    if(product.safety?.cart_blocked||product.select_before_cart){announce(product.safety?.message||'Elige primero la fórmula exacta.');return}
    try{
      await api('/v1/home-food/'+encodeURIComponent(user)+'/pets/'+encodeURIComponent(pet.id)+'/products/'+encodeURIComponent(product.id)+'/shopping',{method:'POST',body:JSON.stringify({confirmed:true})});
      await load({quiet:true});announce(product.brand+' añadido a la lista para revisar; tú confirmarás producto, precio y compra.');
  ...
  function openRecipeImporter(type){
    ...
    $('recipeImportDialogTitle').textContent=pet?'Revisar receta para '+pet.name:type==='image'?'Escanear una receta':'Importar receta';
    $('recipeImageField').hidden=type!=='image';$('recipeUrlField').hidden=type!=='url';$('recipeTextField').hidden=type!=='text';
    $('recipeImportImage').value='';$('recipeImportUrl').value='';$('recipeImportText').value='';
    $('recipeImportPreview').hidden=true;$('recipeImportPreview').replaceChildren();$('recipeImportSave').hidden=true;
    $('recipeImportAnalyze').hidden=false;$('recipeImportAnalyze').disabled=false;$('recipeImportDialog').showModal();
  }
  ...
  function renderImportedRecipe(recipe){const root=$('recipeImportPreview');root.replaceChildren();const title=document.createElement('h3');title.textContent=recipeDisplayTitle(recipe);const description=document.createElement('p');description.textContent=recipe.description||'Revisa los datos antes de guardar.';const facts=document.createElement('dl');[['Ingredientes',(recipe.ingredients||[]).length],['Pasos',(recipe.steps||[]).length],['Porciones',recipe.servings||1]].forEach(([label,value])=>{const dt=document.createElement('dt');dt.textContent=label;const dd=document.createElement('dd');dd.textContent=String(value);facts.append(dt,dd)});root.append(title,description,facts);if(recipe.audience==='pet'){const warning=document.createElement('p');warning.textContent=recipe.veterinary_note||'Consulta al veterinario si esta preparación será parte habitual de su alimentación.';root.append(warning)}root.hidden=false}
  async function analyzeRecipeImport(event){
    event.preventDefault();const type=$('recipeImportType').value;const button=$('recipeImportAnalyze');button.disabled=true;button.textContent='Roxy está leyendo…';
    try{
      let source='';if(type==='image'){const file=$('recipeImportImage').files[0];if(!file)throw new Error('Selecciona una foto, página o captura');source=await imageDataUrl(file)}
      else{source=$(type==='text'?'recipeImportText':'recipeImportUrl').value.trim();if(!source)throw new Error(type==='text'?'Pega los ingredientes y pasos completos':'Escribe el enlace público de la receta')}
      const pet=recipeAudience==='pet'?selectedPetProfile():null;
      const data=await api(`/v1/home-food/${encodeURIComponent(user)}/recipe-imports`,{method:'POST',body:JSON.stringify({source_type:type,source,audience:recipeAudience,pet_species:pet?.species||'',pet_id:pet?.id||''})});
      if(data.status==='NEEDS_CLARIFICATION')throw new Error(data.question);
      pendingImportedRecipe=data.recipe;renderImportedRecipe(data.recipe);
      const ingredients=document.createElement('ul');(data.recipe.ingredients||[]).forEach(item=>{const row=document.createElement('li');row.textContent=`${item.quantity} ${item.unit} · ${item.name}${item.notes?' · '+item.notes:''}`;ingredients.append(row)});
      const steps=document.createElement('ol');(data.recipe.steps||[]).forEach(text=>{const row=document.createElement('li');row.textContent=text;steps.append(row)});
      $('recipeImportPreview').append(ingredients,steps);$('recipeImportSave').hidden=false;$('recipeImportAnalyze').hidden=true;announce('Revisa ingredientes y pasos antes de guardar');
  ...
  function showAllHumanRecipes(){
    ...
    recipeCollection='local';recipeLocalGroup='all';
    selectPanel('recipes');
  }
  ...
  function openRecipe(recipe){
    const catalogPreview=Boolean(recipe.catalog_key&&!recipe.id);currentRecipe=recipe;$('recipeDialogTitle').textContent=recipeDisplayTitle(recipe);$('recipeDialogEyebrow').textContent=catalogPreview?'Vista previa de la receta':'Receta guardada';
    const root=$('recipeDialogContent');root.replaceChildren();
    if(recipe.editorial_status==='needs_canonical_review'&&recipe.audience!=='pet'){
      $('recipeDialogEyebrow').textContent='Propuesta pendiente de revisión';
      const notice=document.createElement('p');notice.textContent='Esta ficha necesita comprobar sus ingredientes y su preparación. No se mostrará como una receta terminada ni se enviarán sus ingredientes a Compra antes de revisarla.';
      const review=makeButton('Explorar recetas originales','primary',()=>{$('recipeDialog').close();recipeCollection='world';selectPanel('recipes');const panel=$('openRecipePanel');const details=panel?.querySelector('details');if(details)details.open=true;panel?.scrollIntoView({block:'start',behavior:'smooth'})});
      root.append(notice,review);$('recipePersonalForm').hidden=true;if(!$('recipeDialog').open)$('recipeDialog').showModal();return;
    }
    const hero=document.createElement('div');hero.className='recipe-detail-hero';const img=document.createElement('img');img.alt=`Ilustración de ${recipeDisplayTitle(recipe)}`;if(recipe.photo_focus)img.style.objectPosition=recipe.photo_focus;hydrateRecipeImage(img,recipe,hero);
    const intro=document.createElement('div');const meta=document.createElement('strong');const recipeLabel=recipe.audience==='pet'?`${recipe.safety_class==='feeding_guide'?'Guía':'Preparación'} para ${({dog:'perros',cat:'gatos',ferret:'ferrets',rabbit:'conejos',guinea_pig:'cobayas',hamster:'hámsteres',small_mammal:'pequeños mamíferos',bird:'aves',fish:'peces',reptile:'reptiles',amphibian:'anfibios',invertebrate:'invertebrados',farm_pet:'mascotas de granja',other:'mascotas'})[recipe.pet_species]||'mascotas'}`:recipe.kind==='drink'?(recipe.drink_type==='alcoholic'?'Bebida con alcohol':'Bebida sin alcohol'):(recipeCategoryLabels[recipeCategoryId(recipe)]||kindLabels[recipe.kind]||'Receta');const servings=Number(recipe.servings||1);const yieldLabel=recipe.audience==="pet"?(recipe.safety_class==="feeding_guide"?"orientación sin porción automática":"cantidades para preparar, no ración diaria"):`${servings} ${servings===1?'porción':'porciones'}`;meta.textContent=`${recipeLabel} · ${yieldLabel}`;
    ...
    if(recipe.audience==='pet'){
      ...
      const imageNote=document.createElement('small');imageNote.textContent='Imagen ilustrativa de la preparación; no indica la cantidad que debe comer tu mascota.';intro.append(imageNote);
    ...
    const sourceReview=recipe.audience==='pet'&&recipe.provenance?.can_cook_from_source!==true;
    if(sourceReview){
    ...
    $('recipePhoto').value='';
    if(!$('recipeDialog').open)$('recipeDialog').showModal();
    if(catalogPreview||sourceReview)videoArea.hidden=true;else loadRecipeVideo(recipe,videoArea);
  }
  ...
  function appendPlantSourceContext(root,plant){
    ...
    [['Espacio para crecer',plant.space],['Si vive en maceta',plant.container_notes],['Sobre esta guía',plant.care_scope]].forEach(([title,value])=>{
      if(!value)return;const row=document.createElement('section'),copy=document.createElement('div'),heading=document.createElement('strong'),body=document.createElement('p');heading.textContent=title;body.textContent=value;copy.append(heading,body);row.append(copy);details.append(row);
    });
  ...
  function openPlantForm(plant=null,flow=null){
    if(window.RoxyGardenGuide){
      ...
      return window.RoxyGardenGuide.open({memberName:activePersonName(),species:homePlants.species||[],plant,readPhoto:readPlantPhoto,onNarration:flow?(step,{explicit=false}={})=>(explicit?flow.repeat:flow.speak)(`garden-${step}`):undefined,stopNarration:flow?.stop,onSave:async(payload)=>{
        ...
        announce(plant?'Condiciones guardadas; tu foto y seguimiento se conservan.':'Planta añadida. Vamos a conocer sus cuidados.');
        return savedResult;
      },onClose:reason=>{if(reason==='saved'){const id=plant?.id||savedResult?.plant?.id;const updated=(homePlants.plants||[]).find(row=>row.id===id);if(updated)openPlantDetail(updated)}}});return;
    }
    const form=$('plantForm');form.reset();form.dataset.plantId=plant?.id||'';
    $('plantDialogTitle').textContent=plant?'Editar planta':'Añadir planta';
    $('plantSubmit').textContent=plant?'Guardar cambios':'Analizar y añadir';
    $('plantPhoto').required=!plant;$('plantPhoto').closest('label').hidden=Boolean(plant);
    $('plantSpecies').disabled=Boolean(plant);$('plantIdentificationNote').hidden=true;
    if(plant){
      const fields={plantName:'display_name',plantSpecies:'species_key',plantRoom:'room',plantPlacement:'placement',plantPot:'pot_type',plantLightExposure:'light_exposure',plantNotes:'notes'};
      Object.entries(fields).forEach(([id,key])=>{$(id).value=plant[key]||''});
      $('plantDrainage').checked=Boolean(plant.drainage);
      if($('plantDetailDialog').open)$('plantDetailDialog').close();
    }
    $('plantDialog').showModal();
  }
  async function submitPlantUpdate(){
    const id=$('plantForm').dataset.plantId,button=$('plantSubmit');button.disabled=true;
    try{
      await api(`/v1/home-plants/${encodeURIComponent(user)}/${encodeURIComponent(id)}`,{method:'PATCH',body:JSON.stringify({display_name:$('plantName').value,room:$('plantRoom').value,placement:$('plantPlacement').value,pot_type:$('plantPot').value,drainage:$('plantDrainage').checked,light_exposure:$('plantLightExposure').value,notes:$('plantNotes').value})});
      $('plantDialog').close();await refreshPlants();
      const updated=(homePlants.plants||[]).find(plant=>plant.id===id);if(updated)openPlantDetail(updated);
      announce('Condiciones actualizadas. Tu foto y seguimiento se conservan.');
    }catch(error){announce(error.message)}finally{button.disabled=false;button.textContent='Guardar cambios'}
  }
  ...
  function plantConditionConcern(plant){return(plant?.condition_concerns||[]).join(' ')}
  function plantWeatherContext(){const current=homeWeather?.current||{};const temperature=typeof current.temperature==='number'?current.temperature:NaN;const ready=familyWeatherAtmosphere().fresh&&Number.isFinite(temperature);const code=current.code==null?NaN:Number(current.code);const rain=ready&&Number.isFinite(code)&&((code>=51&&code<=67)||(code>=80&&code<=82)||code>=95);return{ready,current,temperature,rain,hot:ready&&temperature>=85,cold:ready&&temperature<=50}}
  function renderPlantEnvironment(){
    const weather=plantWeatherContext(),outdoor=(homePlants.plants||[]).some(plant=>plant.placement==='outdoor');
    $('plantTodayDate').textContent=new Intl.DateTimeFormat('es-US',{weekday:'long',day:'numeric',month:'long',year:'numeric'}).format(new Date());
    $('plantWeatherIcon').textContent=weather.ready?(weather.current.icon||'partly_cloudy_day'):'location_off';
    $('plantWeatherTitle').textContent=weather.ready?`${Math.round(weather.temperature)} °F · ${weather.current.condition||'Clima exterior'}`:'Clima exterior sin datos actuales';
    $('plantWeatherDetail').textContent=weather.ready?'Modelo Open-Meteo · no mide la temperatura dentro de tu casa.':'Puedes cuidar tus plantas con tus observaciones, sin compartir ubicación.';
    const environment=homePlants.environment||{};
    const sensorReady=environment.sensor_status==='connected'&&Number.isFinite(environment.temperature_c)&&Number.isFinite(environment.humidity_percent);
    $('plantSensorTitle').parentElement.hidden=!sensorReady;
    if(sensorReady){$('plantSensorTitle').textContent=`${Math.round(environment.temperature_c)} °C · ${Math.round(environment.humidity_percent)}% humedad`;$('plantSensorDetail').textContent='Sensor interior conectado'}
    let advice='Cuéntame cómo están sus hojas y dónde vive. Revisaremos los cuidados paso a paso.';
    if(outdoor&&weather.hot)advice='Para tus plantas de exterior: con calor, comprueba su estado y el sustrato antes de decidir si necesitan agua.';
    else if(outdoor&&weather.rain)advice='Para tus plantas de exterior: comprueba si recibieron lluvia y si la tierra sigue húmeda antes de regar.';
    else if(outdoor&&weather.cold)advice='Para tus plantas de exterior: revisa su rango de temperatura y si necesitan protección del frío.';
  ...
  function renderPlants(){
    ...
    const setupPlants=plants.filter(plant=>(plant.setup_questions||[]).length);
    const plantsDueCount=new Set([...due.map(task=>task.plant_id),...conditionPlants.map(plant=>plant.id)]).size;
    const todayCard=$('todayPlantCareCard');todayCard.hidden=!plants.length;
    $('todayPlantCareTitle').textContent=plantsDueCount?`${plantsDueCount} ${plantsDueCount===1?'planta por revisar':'plantas por revisar'}`:'Sin revisiones programadas hoy';
    $('todayPlantCareDetail').textContent=plantsDueCount?[...new Set([...due.map(row=>row.plant_name),...conditionPlants.map(plant=>plant.display_name)])].slice(0,3).join(' · '):`${plants.length} ${plants.length===1?'planta acompañada':'plantas acompañadas'}`;
    $('plantsTitle').textContent=plants.length?'Tu jardín, acompañado':'Tu jardín empieza aquí';
    $('plantIntro').textContent=plants.length?'Cuidados basados en tus observaciones y el clima; revisa cada planta antes de actuar.':'Añade tu primera planta para conocerla y organizar sus cuidados.';
    $('plantCareTodayTitle').textContent=plantsDueCount?`${plantsDueCount} ${plantsDueCount===1?'planta necesita':'plantas necesitan'} revisión`:plants.length?'Sin cuidados pendientes hoy':'Añade tu primera planta';
    if(setupPlants.length&&!plantsDueCount)$('plantCareTodayTitle').textContent='Conozcamos mejor tu jardín';
    setupPlants.slice(0,2).forEach(plant=>{const row=document.createElement('article');row.className='plant-priority-card';const image=document.createElement('img');image.src=plant.photo_url;image.alt='';const copy=document.createElement('div');copy.innerHTML=`<small>Para personalizar sus cuidados</small><strong>${escapeHtml(plant.display_name)}</strong><p>${escapeHtml(plant.setup_questions.join(' '))}</p>`;row.append(image,copy,makeButton('Completar con Roxy','primary',()=>openPlantForm(plant)));tasksRoot.append(row)});
    conditionPlants.slice(0,2).forEach(plant=>{const row=document.createElement('article');row.className='plant-priority-card';const image=document.createElement('img');image.src=plant.photo_url;image.alt='';const copy=document.createElement('div');copy.innerHTML=`<small>Condiciones a revisar</small><strong>${escapeHtml(plant.display_name)}</strong><p>${escapeHtml(plantConditionConcern(plant))}</p>`;row.append(image,copy,makeButton('Revisar ubicación','secondary',()=>openPlantDetail(plant)));tasksRoot.append(row)});
    if(!due.length&&!conditionPlants.length&&!setupPlants.length){const empty=document.createElement('div');empty.className='plant-priority-empty';empty.innerHTML=plants.length?'<span class="material-symbols-rounded" aria-hidden="true">task_alt</span><div><strong>Sin revisiones programadas hoy</strong><p>El calendario no confirma la salud de la planta. Observa la tierra y las hojas si notas un cambio.</p></div>':'<span class="material-symbols-rounded" aria-hidden="true">photo_camera</span><div><strong>Empieza con una foto</strong><p>Añade una planta para identificarla y crear su plan preventivo.</p></div>';tasksRoot.append(empty)}
    ...
    const health=homePlants.health_summary||{total:plants.length,good:0,watch:0,needs_identification:plants.length};[['Sin pendientes',health.good,'check_circle'],['Por revisar',health.watch,'visibility'],['Por identificar',health.needs_identification,'help']].forEach(([label,count,icon])=>{const item=document.createElement('span');item.innerHTML=`<span class="material-symbols-rounded" aria-hidden="true">${icon}</span><strong>${count}</strong><small>${label}</small>`;healthRoot.append(item)});
    // Care first. No generic soil-meter advert for every plant (including water).
    shopping.hidden=true;
    if(!plants.length){const empty=document.createElement('div');empty.className='plants-empty';empty.innerHTML='<span class="material-symbols-rounded" aria-hidden="true">local_florist</span><strong>Tu jardín empieza con una foto</strong><p>Elige la especie que conoces. Si no la sabes, la ficha quedará por identificar.</p>';collection.append(empty);return}
  ...
  async function addPlantJournal(plant,task=null){
    if(!plant||!window.RoxyGardenGuide?.review)return;
    if($('plantDetailDialog').open)$('plantDetailDialog').close();
    let journalSaved=false,taskSaved=false,accepted=null;
    window.RoxyGardenGuide.review({plant,memberName:activePersonName(),readPhoto:readPlantPhoto,onSave:async(payload)=>{
      if(journalSaved&&JSON.stringify(payload)!==JSON.stringify(accepted))throw new Error('La observación anterior ya está guardada. Conserva ese texto para terminar el cuidado pendiente.');
      const path=`/v1/home-plants/${encodeURIComponent(user)}/${encodeURIComponent(plant.id)}`;
      if(!journalSaved){await api(`${path}/journal`,{method:'POST',body:JSON.stringify({notes:payload.observation,photo_data_url:payload.photo_data_url||'',result:payload.result})});journalSaved=true;accepted={...payload}}
      if(task&&!taskSaved){try{await api(`${path}/tasks/${encodeURIComponent(task.id)}/complete`,{method:'POST',body:JSON.stringify({observation:payload.observation,result:payload.result})});taskSaved=true}catch(error){throw new Error('Tu observación está guardada; falta cerrar la tarea. Reintenta sin cambiar el texto.')}}
      try{await refreshPlants()}catch(_error){announce('Revisión guardada. Vuelve a abrir Jardín para actualizarlo.');return}
      announce(task?'Revisión registrada. El próximo cuidado sigue siendo una comprobación, no un riego obligatorio.':'Observación guardada en su seguimiento.');
  ...
  function familyLocationLabel(member){
    const accuracy=Number(member.location?.accuracy_m);
    return `${member.status||'Ubicación compartida'} · ${Number.isFinite(accuracy)&&accuracy>0?`±${Math.round(accuracy)} m`:'precisión no disponible'}`;
  }
  let familyTrafficLayer=null, familyTrafficEnabled=false;
  function syncFamilyTraffic(notify=false){
    const button=$('familyTrafficToggle');
    try{
      if(familyTrafficEnabled){
        if(!familyMap||!window.google?.maps?.TrafficLayer)throw new Error('Mapa no disponible');
        if(!familyTrafficLayer)familyTrafficLayer=new google.maps.TrafficLayer({autoRefresh:true});
      }
      familyTrafficLayer?.setMap(familyTrafficEnabled?familyMap:null);
      if(notify)announce(familyTrafficEnabled?'Tráfico de Google activado, donde hay cobertura. Se actualiza periódicamente, no al instante.':'Tráfico desactivado');
    }catch(_error){familyTrafficEnabled=false;if(notify)announce('El tráfico no está disponible ahora. Puedes seguir usando el mapa.')}
    button.setAttribute('aria-pressed',String(familyTrafficEnabled));
    button.setAttribute('aria-label',familyTrafficEnabled?'Ocultar tráfico de Google':'Mostrar tráfico de Google');
  }
  ...
  function renderFamilyWeatherFx(){
    ...
    root.dataset.active=String(Boolean(mode));root.style.setProperty('--weather-cloud-opacity',String(scene.clouds===null?.12:.04+scene.clouds*.001));root.style.setProperty('--weather-cloud-duration',`${Math.max(12,48-(scene.wind||0))}s`);
    if(window.RoxyWeatherRenderer)window.RoxyWeatherRenderer.render(root,scene,{paused});
    else{root.replaceChildren();root.dataset.weatherMotion='off'}
  ...
  async function activateFamilyWeatherGlobe(){
    ...
    try{
      ...
      console.warn('Visor meteorológico no disponible',error);
    }
  }
  ...
  function clearFamilyRoutes(){familyRouteRenderGeneration++;familyRoutes.forEach(route=>route.setMap(null));familyRoutes=[];if(familyDirectionsRenderer){familyDirectionsRenderer.setMap(null);familyDirectionsRenderer=null}familyRouteSnapshot=null;const traffic=$('familyTrafficSummary');if(traffic)traffic.textContent='Selecciona una ruta'}
  ...
  function renderFamilyExperience(){
    ...
    rows.forEach(member=>{
      ...
      const device=document.createElement('small');device.textContent=member.presence_label||(member.sharing_enabled?'Última ubicación':'Ubicación privada');
    ...
    if(!member.sharing_enabled||!member.location){
      focus.replaceChildren();focus.append(familyAvatarNode(member,'family-focus-avatar'));
      const copy=document.createElement('div');copy.className='family-focus-copy';
      copy.innerHTML=`<h4>${escapeHtml(member.display_name||'Miembro')}</h4><p>${member.sharing_enabled?'Esperando su primera ubicación.':'Su ubicación es privada. Sigue formando parte de tu Nexo.'}</p>`;
      if(member.is_viewer){const button=makeButton('Revisar mi privacidad','secondary',()=>{$('familySettings').open=true;$('familySettings').scrollIntoView({block:'start',behavior:'smooth'})});copy.append(button)}
      focus.append(copy);route.hidden=true;return;
    }
    ...
    const actions=document.createElement('div');actions.className='family-focus-actions';actions.innerHTML='<button type="button" class="primary" data-family-leave><span class="material-symbols-rounded" aria-hidden="true">location_on</span>Ver lugares guardados</button><button type="button" class="secondary" data-family-route><span class="material-symbols-rounded" aria-hidden="true">route</span>Ver camino</button>';
    actions.querySelector('[data-family-leave]').addEventListener('click',()=>{$('familySettings').open=true;$('familyPlaceName').focus();announce('Los avisos de salida de otra persona aún no están disponibles. Aquí puedes revisar Casa y Trabajo.')});
  ...
  async function renderFamilyRouteCard(member){
    const generation=++familyRouteRenderGeneration;
    const identity=()=>JSON.stringify([user,account.mode,account.id||'',account.storage_user_id||'']);
    ...
    const currentMember=()=> (homeFamily.members||[]).find(row=>String(row.id)===String(member?.id));
    const locationKey=row=>row?.location?JSON.stringify([row.location.latitude,row.location.longitude,row.location.recorded_at||row.updated_at||'']):'';
    const eventKey=row=>row?JSON.stringify([row.id||'',row.location||'',row.starts_at||row.start||'']):'';
    const validLocation=row=>Boolean(row?.sharing_enabled&&row.location&&['latitude','longitude'].every(key=>row.location[key]!==null&&row.location[key]!==''&&Number.isFinite(Number(row.location[key]))));
    const latest=currentMember();member=latest||member;
    const initialLocation=locationKey(latest),initialEvent=eventKey(event);
    const isCurrent=()=>generation===familyRouteRenderGeneration&&activePanel==='family'&&account.mode==='member'&&identity()===initialIdentity&&familyRouteMode&&String(familySelectedMember()?.id)===String(member?.id)&&validLocation(currentMember())&&locationKey(currentMember())===initialLocation&&eventKey(familyNextCalendarEvent())===initialEvent;
    const hideRoute=()=>{if(generation!==familyRouteRenderGeneration)return;if(root)root.hidden=true;if(focus)focus.hidden=false;sheet?.classList.remove('is-route-mode');if(familyDirectionsRenderer){familyDirectionsRenderer.setMap(null);familyDirectionsRenderer=null}familyRouteSnapshot=null;const traffic=$('familyTrafficSummary');if(traffic)traffic.textContent='Selecciona una ruta'};
    if(!root||!event||!isCurrent()){hideRoute();return}
  ...
  async function startFamilyLocationWatcher({automatic=false}={}){
    ...
    try{
      ...
      return true;
    }catch(error){if(familyWatchId!==null)navigator.geolocation.clearWatch(familyWatchId);familyWatchId=null;$('familyMapStatus').textContent=(homeFamily.members||[]).find(row=>row.is_viewer)?.sharing_enabled?'Tu preferencia sigue activa, pero falta permiso del navegador para actualizar.':'No se activó la ubicación. Tu posición sigue privada.';if(!automatic)announce(error.message);return false}
  ...
  function renderDesignConnections(){
    ...
    const icons={walmart_affiliate:'storefront',ebay_browse:'sell',best_buy_products:'tv',impact:'hub',cj_affiliate:'join_inner',awin:'account_tree',amazon_associates:'shopping_bag',amazon_creators:'package_2',pinterest_trends:'trending_up',dataforseo_merchant:'compare_arrows'};
  ...
  function designBudgetSummary(project){
    const limit=Math.max(0,Number(project.budget)||0);
    const selected=(project.budget_tiers||[]).find(row=>row.id===(project.selected_tier||'balanced'));
    const target=Math.max(0,Number(selected?.budget??limit)||0);
    const difference=Math.max(0,Math.round((target-limit)*100)/100);
    const money=value=>new Intl.NumberFormat('en-US',{style:'currency',currency:'USD'}).format(value);
    return{limit,target,difference,overLimit:difference>0,ratio:limit?Math.round(target/limit*100):0,
      title:difference>0?'Esta opción supera tu presupuesto':'Objetivo dentro de tu presupuesto',
      detail:`Objetivo ${money(target)} · límite ${money(limit)}. ${difference>0?`Supera el límite por ${money(difference)}. `:''}Reparto orientativo, no cotización; faltan precios, impuestos y envío.`};
  }
  function designProjectPreview(project){
    const id=designPreviewTiers.get(`${user}:${project.id}`);
    const tier=(project.budget_tiers||[]).find(row=>row.id===id);
  ...
  function renderDesign(){
    ...
    projects.forEach(savedProject=>{
      const project=designProjectPreview(savedProject);
      ...
      const tierTabs=document.createElement('div');tierTabs.className='design-tier-tabs';tierTabs.setAttribute('aria-label','Nivel de presupuesto');(project.budget_tiers||[]).forEach(tier=>{const button=document.createElement('button');button.type='button';button.classList.toggle('active',tier.id===(project.selected_tier||'balanced'));button.setAttribute('aria-pressed',String(tier.id===(project.selected_tier||'balanced')));button.innerHTML=`<strong>${tier.label}</strong><small>$${Number(tier.budget||0).toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2})}</small>`;button.addEventListener('click',()=>{designPreviewTiers.set(`${user}:${project.id}`,tier.id);renderDesign()});tierTabs.append(button)});
      const budgetState=designBudgetSummary(project);const budgetPlan=document.createElement('section');budgetPlan.className='design-budget-plan';budgetPlan.innerHTML=`<div><span><strong>${budgetState.title}</strong><small>${budgetState.detail}</small></span><b>${project.selected_tier==='economy'?'Esencial':project.selected_tier==='complete'?'Completo':'Equilibrado'}</b></div><meter aria-label="Objetivo comparado con tu límite de presupuesto" min="0" max="${Math.max(100,budgetState.ratio)}" high="100" optimum="65" value="${budgetState.ratio}">${budgetState.ratio}% del límite</meter>`;budgetPlan.append(tierTabs);
      ...
      const analysis=document.createElement('section');analysis.className='design-analysis';const analysisHeading=document.createElement('div');const analysisTitle=document.createElement('strong');analysisTitle.textContent=project.analysis_status==='READY_AI'?'Análisis de la foto':'Ideas basadas en tus respuestas';const analyze=makeButton((project.analysis_status==='READY_AI'?'Reanalizar y rediseñar':'Analizar y rediseñar'),'secondary',()=>analyzeDesignProject(project.id));analyze.disabled=!homeDesign.generation_configured;analysisHeading.append(analysisTitle,analyze);const analysisCopy=document.createElement('p');analysisCopy.textContent=project.analysis_status==='READY_AI'?(project.analysis?.summary||'Análisis disponible'):'Estas ideas usan lo que escribiste, no un análisis de la foto. Confirma medidas y revisa cada opción antes de elegir.';analysis.append(analysisHeading,analysisCopy);const insights=document.createElement('ul');[...(project.analysis?.strengths||[]),...(project.analysis?.opportunities||[]),...(project.analysis?.questions||[])].slice(0,6).forEach(value=>{const item=document.createElement('li');item.textContent=value;insights.append(item)});if(insights.children.length)analysis.append(insights);
  ...
  async function login(event){
    ...
    try{
      const result=await api('/v1/home-account/login',{method:'POST',body:JSON.stringify({username:$('loginUsername').value.trim(),password:$('loginPassword').value})});
      account=result;user=result.storage_user_id;localStorage.setItem('roxyShoppingUser',user);$('loginPassword').value='';
      // Changing only #hoy does not reload a document or dismiss a stale sign-in dialog.
  ...
  function ensureRoxyTextSpeech(){
    ...
    ($('roxyVoicePrivacy')||$('roxyVoicePanel')).append(button);return button;
  }
  function ensureRoxyOfficialListen(){
    let button=$('roxyOfficialListen');if(button)return button;
    button=document.createElement('button');button.id='roxyOfficialListen';button.type='button';button.className='secondary';button.textContent='Escuchar con Roxy';button.hidden=true;
    button.addEventListener('click',()=>{const response=roxyReadableResponse;if(response&&response.owner===user&&response.identity===collectionIdentity())void speakRoxyGeneral(response.text);});
    $('roxyVoicePanel').append(button);return button;
  }
  function createHomeVoiceController(attempt,owner,identity){
    const current=()=>attempt===roxyVoiceAttempt&&user===owner&&collectionIdentity()===identity&&!$('roxyVoicePanel').hidden;
    const memberId=account.id;
    return window.RoxyHomeConversation.create({isCurrent:current,
      sendCommand:(command,signal)=>sendRoxyHomeCommand({command,signal,isCurrent:current}),
      requestSpeech:async(text,signal)=>{
        if(!current())throw new Error('La sesión cambió.');
        const response=await fetch(`/v1/assistant/speech/${encodeURIComponent(owner)}`,{method:'POST',credentials:'include',cache:'no-store',signal,
          headers:{'Content-Type':'application/json','X-Roxy-Recipe-Member':memberId},body:JSON.stringify({text})});
        if(!current())throw new Error('La sesión cambió.');
        if(!response.ok){let detail;try{detail=(await response.json()).detail}catch(_){}throw new Error(typeof detail==='string'?detail:detail?.message||'No pude conectar la voz oficial. Puedes reintentar la lectura.');}
        const blob=await response.blob();if(!current())throw new Error('La sesión cambió.');
        if(blob.type!=='audio/mpeg'||blob.size<1024||blob.size>10*1024*1024)throw new Error('El audio no es válido. Puedes reintentar.');return blob;
      },onStatus:roxyVoiceStatus,onTranscript:roxyVoiceTranscript,
      onEnd:()=>{if(!current())return;roxyVoiceConversation=null;roxyVoiceStarting=false;$('roxyVoiceStart').disabled=false;$('roxyVoiceEnd').disabled=true;},
    });
  }
  async function speakRoxyGeneral(text){
    if(!window.RoxyHomeConversation||$('roxyVoicePanel').hidden)return;
    const owner=user,identity=collectionIdentity();
    await endRoxyVoice();stopCookingSpeech();window.RoxyRecipeGuide?.pauseActive?.();
    if(user!==owner||collectionIdentity()!==identity||$('roxyVoicePanel').hidden||document.hidden)return;
    const attempt=++roxyVoiceAttempt;
    roxyVoiceConversation=createHomeVoiceController(attempt,owner,identity);$('roxyVoiceStart').disabled=true;$('roxyVoiceEnd').disabled=false;
  ...
  function roxyVoiceError(error,phase){
    ...
    return'No pude conectar la voz. Puedes volver a intentar o escribirle a Roxy.';
  }
  async function sendRoxyText(event){
    ...
    const owner=user,identity=collectionIdentity();
    button.disabled=true;await endRoxyVoice();const attempt=roxyVoiceAttempt;
    const current=()=>user===owner&&collectionIdentity()===identity&&attempt===roxyVoiceAttempt&&!$('roxyVoicePanel').hidden;
    if(!current()){button.disabled=false;return;}roxyVoiceTranscript(command,'Tú');roxyVoiceStatus('Roxy está pensando…');
    try{const result=await sendRoxyHomeCommand({command,isCurrent:current});if(!current())return;input.value='';const answer=result.speech||result.message||'No recibí una respuesta. Intenta de nuevo.';roxyVoiceTranscript(answer);roxyVoiceStatus('Respuesta del agente de Home.');void speakRoxyGeneral(answer)}
  ...
  async function startRoxyVoice(){
    ...
    try{
      ...
      if(!current())return;
      if(config.status==='CONFIGURED'&&config.voice_mode==='home_turns'&&config.agent==='roxy_home'){
        if(!window.RoxyHomeConversation)throw new Error('No se cargó el agente de Home');
        phase='micrófono';roxyVoiceStarting=false;roxyVoiceConversation=createHomeVoiceController(attempt,owner,identity);
        $('roxyVoiceEnd').disabled=false;roxyVoiceConversation.start();return;
      }
      ...
      phase='conexión de voz';
      const options={connectionType:'websocket',overrides:roxyHomeOverrides(),dynamicVariables:{...(config.dynamic_variables||{}),shopping_list_json:JSON.stringify(currentShoppingSummary())},clientTools:roxyHomeClientTools(),
        onConnect:()=>{if(!current())return;roxyVoiceStarting=false;roxyVoiceStatus('Roxy te está escuchando');$('roxyVoiceStart').disabled=true;$('roxyVoiceEnd').disabled=false},
        onDisconnect:details=>{if(!current())return;release();roxyVoiceConversation=null;roxyVoiceStarting=false;roxyVoiceAttempt+=1;roxyVoiceStatus(details?.reason==='agent'?'La llamada terminó. Puedes reconectar o escribir.':'Conversación terminada');$('roxyVoiceStart').disabled=false;$('roxyVoiceEnd').disabled=true},
        onError:error=>{if(!current())return;release();const message=roxyVoiceError(error,'conversación');void endRoxyVoice().then(()=>{if(roxyVoiceAttempt===attempt+1)roxyVoiceStatus(message,true)})},
  ...
  function bind(){
    ...
    $('newPlantButton').addEventListener('click',()=>openPlantForm());
    $('plantForm').addEventListener('submit',event=>{if(event.currentTarget.dataset.plantId){event.preventDefault();void submitPlantUpdate()}else void submitPlant(event)});
    ...
    $('petProfileSpecies').addEventListener('change',()=>{adaptPetQuestions();const species=$('petProfileSpecies').value;$('petProfileExactSpecies').placeholder=({bird:'Ej. Canario o periquito australiano',fish:'Ej. Betta splendens',reptile:'Ej. Gecko leopardo',ferret:'Ferret / hurón doméstico'})[species]||'Nombre común o científico';});
    ...
    document.querySelectorAll('[data-pet-hub-tab]').forEach(button=>{button.addEventListener('click',()=>{petHubTab=button.dataset.petHubTab;renderRecipes()});button.addEventListener('keydown',event=>{if(!['ArrowLeft','ArrowRight','Home','End'].includes(event.key))return;event.preventDefault();const tabs=[...document.querySelectorAll('[data-pet-hub-tab]')].filter(tab=>!tab.hidden);const current=tabs.indexOf(button);const target=event.key==='Home'?0:event.key==='End'?tabs.length-1:(current+(event.key==='ArrowRight'?1:-1)+tabs.length)%tabs.length;tabs[target].click();tabs[target].focus()})});
    ...
    $('roxyTextForm').addEventListener('submit',sendRoxyText);
