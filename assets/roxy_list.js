(() => {
  ...
  function renderPetProducts(pet){
    ...
    rows.forEach((product,index)=>{const card=document.createElement('article');card.className='pet-product-card';if(product.image_url){const image=document.createElement('img');image.className='pet-product-photo';image.src=product.image_url;image.alt=`${product.brand} ${product.name}`;image.loading='lazy';card.append(image)}const badge=document.createElement('span');badge.className='pet-product-rank';badge.textContent=(petProductFilter==='all'&&index===0?'Mejor para '+pet.name+' · ':'')+`${product.score||50}% compatible`;const copy=document.createElement('div');copy.className='pet-product-copy';const brand=document.createElement('small');brand.textContent=product.brand;const title=document.createElement('h4');title.textContent=product.name;const category=document.createElement('strong');category.textContent=product.category;const reasonLabel=document.createElement('b');reasonLabel.textContent='Por qué lo recomienda Roxy';const reason=document.createElement('p');reason.textContent=product.reason;copy.append(brand,title,category,reasonLabel,reason);if(product.requires_vet||product.select_before_cart){const warning=document.createElement('em');warning.textContent=product.select_before_cart?'Elige primero la fórmula exacta en el catálogo oficial':'Revisar con su veterinario antes de cambiar o comprar';copy.append(warning)}const actions=document.createElement('div');actions.className='pet-product-actions';const source=document.createElement('a');source.href=product.source_url;source.target='_blank';source.rel='noopener noreferrer';source.textContent=product.select_before_cart?'Elegir fórmula oficial':'Información oficial';actions.append(source);if(!product.select_before_cart)actions.append(makeButton(product.requires_vet?'Añadir para revisar':'Añadir al carrito','primary',()=>addPetProduct(product,pet)));card.append(badge,copy,actions);root.append(card)});
