(() => {
  ...
  function renderRecipes() {
    ...
    const pets=savedPets();const onboarding=$('petOnboardingEmpty');const catalogSection=$('recipeCatalogSection');const importStudio=$('recipeImportStudio');
    if(recipeAudience==='pet'&&!pets.length){onboarding.hidden=false;catalogSection.hidden=true;importStudio.classList.add('pet-onboarding-mode');return}
    onboarding.hidden=true;catalogSection.hidden=false;importStudio.classList.remove('pet-onboarding-mode');
    ...
    const pet=selectedPetProfile();const blockedIngredients=(pet?.allergies||[]).map(normalize).filter(Boolean);
  ...
  function openPetProfile(){petProfileStep=1;petProfilePhotoData='';$('petProfileForm').reset();$('petPhotoPreview').replaceChildren();const icon=document.createElement('span');icon.className='material-symbols-rounded';icon.textContent='add_a_photo';const label=document.createElement('strong');label.textContent='Añadir foto';$('petPhotoPreview').append(icon,label);renderPetProfileStep();$('petProfileDialog').showModal()}
  ...
  function bind(){
    ...
    $('petProfileSpecies').addEventListener('change',adaptPetEnvironment);
