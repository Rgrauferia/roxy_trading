(() => {
  ...
  function renderRecipes() {
    ...
    const pet=selectedPetProfile();const blockedIngredients=(pet?.allergies||[]).map(normalize).filter(value=>value&&!/^ninguna\b/.test(value));
    const petIdentity=normalize(`${pet?.exact_species||''} ${pet?.breed||''}`);const audienceRows=allRows.filter(recipe=>{if(recipeAudience!=='pet')return recipe.audience!=='pet';if(recipe.audience!=='pet'||(recipe.pet_species&&recipe.pet_species!==petSpecies))return false;const exactTerms=(recipe.pet_exact_terms||[]).map(normalize).filter(Boolean);if(exactTerms.length&&!exactTerms.some(term=>petIdentity.includes(term)))return false;const ingredients=normalize((recipe.ingredients||[]).map(row=>typeof row==='string'?row:row.name||'').join(' '));return !blockedIngredients.some(item=>ingredients.includes(item))});
  ...
  function renderPetProfileCompletion(pet){const root=$('petProfileCompletion');root.replaceChildren();const completion=(homeFood.pet_profile_completions||{})[String(pet.id)]||{percent:0,missing:[],next_step:1};root.classList.toggle('complete',completion.status==='complete');const score=document.createElement('span');score.className='pet-completion-score';score.textContent=completion.percent+'%';const copy=document.createElement('div');const title=document.createElement('strong');title.textContent=completion.status==='complete'?'Perfil completo y listo':`Completa ${completion.missing.length} ${completion.missing.length===1?'dato':'datos'} para afinar Roxy`;const detail=document.createElement('p');detail.textContent=completion.status==='complete'?'Roxy tiene identidad, salud, alimentación, entorno y rutina confirmados.':(completion.missing||[]).slice(0,2).map(item=>item.label).join(' · ');copy.append(title,detail);root.append(score,copy);if(completion.status!=='complete'){const button=makeButton('Completar ahora','secondary',()=>{openPetProfile(pet);petProfileStep=Number(completion.next_step||1);renderPetProfileStep()});root.append(button)}}
  function renderPetHub(){const pet=selectedPetProfile();if(!pet)return;$('petHubTitle').textContent='Todo para '+pet.name;const details=[petSpeciesLabels[pet.species]||'Mascota',pet.breed||pet.exact_species,pet.life_stage&&pet.life_stage!=='unknown'?({baby:'Bebé',young:'Joven',adult:'Adulto',senior:'Senior'})[pet.life_stage]:''].filter(Boolean);$('petHubSummary').textContent=details.join(' · ');renderPetProfileCompletion(pet);$('petFoodTab').textContent=['fish','reptile','amphibian','invertebrate','farm_pet','other'].includes(pet.species)?'Guías':'Recetas';document.querySelectorAll('[data-pet-hub-tab]').forEach(button=>{const active=button.dataset.petHubTab===petHubTab;button.classList.toggle('active',active);button.setAttribute('aria-selected',String(active));button.tabIndex=active?0:-1});document.querySelectorAll('[data-pet-hub-panel]').forEach(panel=>panel.hidden=panel.dataset.petHubPanel!==petHubTab);if(petHubTab==='care')renderPetCare(pet);if(petHubTab==='nutrition')renderPetNutrition(pet);if(petHubTab==='products')renderPetProducts(pet);if(petHubTab==='medical')renderPetMedicalHistory(pet)}
  ...
  function renderPetDailySummary(pet,plan){
    ...
    const hero=document.createElement('section');hero.className='pet-today-card';
    const identity=document.createElement('div');identity.className='pet-today-identity';
    const avatar=document.createElement('span');avatar.className='pet-today-avatar';
    if(pet.photo_data_url){const image=document.createElement('img');image.src=pet.photo_data_url;image.alt='';avatar.append(image)}else{const icon=document.createElement('span');icon.className='material-symbols-rounded';icon.setAttribute('aria-hidden','true');icon.textContent='pets';avatar.append(icon)}
    const heading=document.createElement('div');const eyebrow=document.createElement('small');eyebrow.textContent='Hoy con '+pet.name;const title=document.createElement('strong');title.textContent=completed===daily.length&&daily.length?'Rutina diaria completada':`${completed} de ${daily.length} cuidados diarios`;heading.append(eyebrow,title);identity.append(avatar,heading);
    const progress=document.createElement('span');progress.className='pet-today-progress';progress.textContent=daily.length?Math.round(completed/daily.length*100)+'%':'Plan listo';hero.append(identity,progress);root.append(hero);
    if(next){const nextCard=document.createElement('article');nextCard.className='pet-next-routine';const icon=document.createElement('span');icon.className='material-symbols-rounded';icon.setAttribute('aria-hidden','true');icon.textContent=next.icon||'event_repeat';const copy=document.createElement('div');const label=document.createElement('small');label.textContent=(next.cadence==='weekly'?'Esta semana':'Próximo')+' · '+petRoutineTimeLabel(next.time);const nextTitle=document.createElement('strong');nextTitle.textContent=next.title;const detail=document.createElement('p');detail.textContent=next.detail;copy.append(label,nextTitle,detail);nextCard.append(icon,copy);root.append(nextCard)}
    const list=document.createElement('div');list.className='pet-routine-list';routines.forEach(routine=>{const row=document.createElement('article');row.className='pet-routine-row'+(routine.completed_today?' completed':'');const time=document.createElement('time');time.textContent=petRoutineTimeLabel(routine.time);const copy=document.createElement('div');const title=document.createElement('strong');title.textContent=routine.title;const meta=document.createElement('small');meta.textContent=routine.cadence==='weekly'?'Semanal':routine.detail;copy.append(title,meta);const button=document.createElement('button');button.type='button';button.className=routine.completed_today?'secondary':'primary';button.disabled=routine.completed_today;button.textContent=routine.completed_today?'Hecho ✓':'Registrar';button.addEventListener('click',()=>completePetRoutine(pet,routine,button));row.append(time,copy,button);list.append(row)});root.append(list);
  ...
  function renderPetCare(pet){const plan=(homeFood.pet_care_plans||{})[String(pet.id)]||{};$('petCareTitle').textContent=plan.title||'Plan de cuidado';$('petCareIntro').textContent=plan.intro||'Completa el perfil para personalizar sus cuidados.';renderPetDailySummary(pet,plan);const root=$('petCarePlan');root.replaceChildren();(plan.sections||[]).forEach(section=>{const card=document.createElement('article');card.className='pet-care-card'+(section.urgent?' urgent':'')+(section.protected?' protected':'');const icon=document.createElement('span');icon.className='material-symbols-rounded';icon.setAttribute('aria-hidden','true');icon.textContent=section.icon||'pets';const copy=document.createElement('div');const title=document.createElement('h4');title.textContent=section.title;const text=document.createElement('p');text.textContent=section.text;copy.append(title,text);card.append(icon,copy);root.append(card)});if(plan.needs_exact_species){const notice=document.createElement('button');notice.type='button';notice.className='pet-care-exact';notice.innerHTML='<span class="material-symbols-rounded" aria-hidden="true">edit</span><span><strong>Falta la especie exacta</strong><small>Añádela para afinar parámetros, alimentación y convivencia.</small></span>';notice.addEventListener('click',()=>openPetProfile(pet));root.prepend(notice)}if(plan.source_url){const source=document.createElement('a');source.className='pet-care-source';source.href=plan.source_url;source.target='_blank';source.rel='noopener noreferrer';source.textContent='Fuente de cuidado: '+plan.source_label;root.append(source)}if(plan.legal_note){const legal=document.createElement('p');legal.className='pet-care-legal';legal.textContent=plan.legal_note;root.append(legal)}}
  async function logPetFeeding(pet,outcome,button){const labels={all:'Comió todo',partial:'Comió parte',refused:'No quiso comer'};button.disabled=true;try{await api(`/v1/home-food/${encodeURIComponent(user)}/pets/${encodeURIComponent(pet.id)}/care-log`,{method:'POST',body:JSON.stringify({routine_id:'feeding_observation',title:'Registro de alimentación',outcome})});await load({quiet:true});petHubTab='nutrition';renderRecipes();announce(labels[outcome]+' registrado para '+pet.name)}catch(error){button.disabled=false;announce(error.message)}}
  ...
  function renderPetProducts(pet){const root=$('petProductRecommendations');root.replaceChildren();const rows=(homeFood.pet_recommendations||{})[String(pet.id)]||[];rows.forEach((product,index)=>{const card=document.createElement('article');card.className='pet-product-card';const badge=document.createElement('span');badge.className='pet-product-rank';badge.textContent=index===0?'Mejor coincidencia':'Alternativa verificada';const copy=document.createElement('div');const brand=document.createElement('small');brand.textContent=product.brand;const title=document.createElement('h4');title.textContent=product.name;const category=document.createElement('strong');category.textContent=product.category;const reason=document.createElement('p');reason.textContent=product.reason;copy.append(brand,title,category,reason);if(product.requires_vet||product.select_before_cart){const warning=document.createElement('em');warning.textContent=product.select_before_cart?'Elige primero la fórmula exacta en el catálogo oficial':'Revisar con su veterinario antes de cambiar o comprar';copy.append(warning)}const actions=document.createElement('div');const source=document.createElement('a');source.href=product.source_url;source.target='_blank';source.rel='noopener noreferrer';source.textContent=product.select_before_cart?'Elegir fórmula oficial':'Ver fuente oficial';actions.append(source);if(!product.select_before_cart)actions.append(makeButton(product.requires_vet?'Añadir para revisar':'Añadir a mi lista','primary',()=>addPetProduct(product,pet)));card.append(badge,copy,actions);root.append(card)});if(!rows.length){const empty=document.createElement('div');empty.className='empty';empty.innerHTML=pet.exact_species?'<strong>Aún no hay un producto verificado para esta especie</strong>Roxy no inventará una marca. Conserva el plan de cuidado y consulta a un veterinario de exóticos para elegir el producto correcto.':'<strong>Completa la especie exacta</strong>Roxy la necesita antes de recomendar un producto concreto.';root.append(empty)}}
  ...
  function renderPlants(){
    const collection=$('plantCollection');const tasksRoot=$('plantTasksToday');if(!collection||!tasksRoot)return;collection.replaceChildren();tasksRoot.replaceChildren();populatePlantSpecies();
    const plants=homePlants.plants||[];const due=homePlants.due_today||[];$('plantCount').textContent=`${plants.length} ${plants.length===1?'planta':'plantas'}`;
    ...
    if(!due.length){const empty=document.createElement('p');empty.className='plant-empty-note';empty.textContent=plants.length?'No hay cuidados vencidos. Sigue observando luz, tierra y hojas.':'Añade una planta con una foto para comenzar.';tasksRoot.append(empty)}
    due.forEach(task=>{const row=document.createElement('article');row.className='plant-task';const image=document.createElement('img');image.src=task.photo_url;image.alt='';const copy=document.createElement('span');copy.innerHTML=`<strong>${escapeHtml(task.plant_name)}</strong><small>${escapeHtml(task.title||'Revisar planta')} · ${escapeHtml(task.due_date||'hoy')}</small>`;const done=makeButton('Hecho','primary',async()=>{const observation=window.prompt('¿Qué observaste? Ej. tierra seca, regué y dejé drenar','')||'';try{await api(`/v1/home-plants/${encodeURIComponent(user)}/${encodeURIComponent(task.plant_id)}/tasks/${encodeURIComponent(task.id)}/complete`,{method:'POST',body:JSON.stringify({observation})});await refreshPlants();announce('Cuidado registrado para todo el hogar')}catch(error){announce(error.message)}});const remind=makeButton('Recordar','secondary',()=>createPlantReminder(task));row.append(image,copy,done,remind);tasksRoot.append(row)});
  ...
  function openPlantDetail(plant){currentPlant=plant;$('plantDetailTitle').textContent=plant.display_name;const root=$('plantDetailContent');root.replaceChildren();const hero=document.createElement('div');hero.className='plant-detail-hero';const image=document.createElement('img');image.src=plant.photo_url;image.alt=`Foto de ${plant.display_name}`;const copy=document.createElement('span');copy.innerHTML=`<strong>${escapeHtml(plant.common_name||'Especie por confirmar')}</strong><em>${escapeHtml(plant.scientific_name||'')}</em><small>${escapeHtml(plant.room||'Sin ubicación')} · ${plant.placement==='outdoor'?'Exterior':'Interior'}</small>`;hero.append(image,copy);root.append(hero);
    ...
    const facts=document.createElement('div');facts.className='plant-facts';[['Luz','light_mode',plant.light],['Cuándo revisar','water_drop',plant.soil_rule],['Fertilizante','nutrition',plant.fertilizer],['Historia','history_edu',plant.history],['Mascotas',plant.pet_safe?'pets':'warning',plant.toxicity]].forEach(([title,icon,value])=>{const row=document.createElement('section');row.innerHTML=`<span class="material-symbols-rounded" aria-hidden="true">${icon}</span><div><strong>${title}</strong><p>${escapeHtml(value||'Información no confirmada')}</p></div>`;facts.append(row)});root.append(facts);
    const journal=plant.journal||[];if(journal.length){const history=document.createElement('section');history.className='plant-journal';history.innerHTML='<h3>Historia de crecimiento</h3>';[...journal].reverse().forEach(entry=>{const row=document.createElement('article');if(entry.photo_url){const photo=document.createElement('img');photo.src=entry.photo_url;photo.alt=`Evolución de ${plant.display_name}`;row.append(photo)}const copy=document.createElement('div');const stamp=new Date(entry.created_at||'');copy.innerHTML=`<strong>${Number.isNaN(stamp.getTime())?'Registro':stamp.toLocaleDateString('es-US',{day:'numeric',month:'long',year:'numeric'})}</strong><p>${escapeHtml(entry.notes||'Sin observaciones')}</p>`;row.append(copy);history.append(row)});root.append(history)}
    const actions=document.createElement('div');actions.className='plant-detail-actions';actions.append(makeButton('Añadir foto de crecimiento','secondary',()=>addPlantJournal(plant)),makeButton('Añadir cuidados a compra','secondary',()=>addPlantProduct(plant)),makeButton('Eliminar planta','danger-button',()=>deletePlant(plant)));root.append(actions);$('plantDetailDialog').showModal()}
  async function addPlantJournal(plant){const picker=document.createElement('input');picker.type='file';picker.accept='image/jpeg,image/png,image/webp';picker.capture='environment';picker.addEventListener('change',async()=>{if(!picker.files?.[0])return;const notes=window.prompt('¿Qué cambió desde la última foto?','')||'Foto de crecimiento';try{const photo=await readPlantPhoto(picker.files[0]);await api(`/v1/home-plants/${encodeURIComponent(user)}/${encodeURIComponent(plant.id)}/journal`,{method:'POST',body:JSON.stringify({notes,photo_data_url:photo})});await refreshPlants();const updated=(homePlants.plants||[]).find(row=>row.id===plant.id);if(updated)openPlantDetail(updated);announce('Foto guardada en su historia de crecimiento')}catch(error){announce(error.message)}});picker.click()}
  async function addPlantProduct(plant){const name=`Fertilizante para ${plant.common_name||plant.display_name}`;try{await api(`/v1/shopping/${encodeURIComponent(user)}`,{method:'POST',body:JSON.stringify({name,quantity:1,unit:'unidad',category:'HOUSEHOLD',notes:'Revisar etiqueta y compatibilidad antes de comprar.'})});await load({quiet:true});$('plantDetailDialog').close();selectPanel('shopping');announce('Producto añadido para comparar en tu lista')}catch(error){announce(error.message)}}
  ...
  function renderDesign(){
    ...
    $('designGenerationNotice').textContent=homeDesign.generation_configured?'Roxy analiza tu foto privada y genera solo la opción de presupuesto que elijas. Los precios se verifican al abrir cada comercio.':'Puedes guardar proyectos y comparar presupuestos; el análisis y la propuesta visual necesitan la clave privada de OpenAI de Home.';
    ...
    projects.forEach(project=>{
      ...
      const body=document.createElement('div');body.className='design-project-body';const heading=document.createElement('div');heading.className='design-project-heading';const copy=document.createElement('div');const title=document.createElement('h3');title.textContent=project.name;const meta=document.createElement('p');meta.textContent=`${project.room_label} · ${project.style_label}${project.measurements?` · ${project.measurements}`:''}`;copy.append(title,meta);heading.append(copy);
      ...
      const actions=document.createElement('div');actions.className='design-project-actions';const generate=makeButton(project.proposal_status==='FAILED'?'Intentar rediseño otra vez':project.proposal_url?'Actualizar rediseño completo':'Crear rediseño completo','primary',()=>generateDesignProposal(project.id,project.selected_tier));generate.disabled=project.proposal_status==='GENERATING'||!homeDesign.generation_configured;const buy=makeButton('Comparar muebles reales','secondary',()=>prepareDesignPurchase(project,card));const remove=makeButton('Eliminar proyecto','danger-button',()=>deleteDesignProject(project));actions.append(generate,buy,remove);
      body.append(heading,tierTabs,analysis);if(recommendationRows.length)body.append(recommendations);body.append(fit,products,revision,actions);card.append(visuals);if(comparisonControl)card.append(comparisonControl);card.append(body);root.append(card);
    });
  }
  ...
  function bind(){
    ...
    $('editPetNutrition').addEventListener('click',()=>{openPetProfile(selectedPetProfile());petProfileStep=3;renderPetProfileStep()});
