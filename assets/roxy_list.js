(() => {
  ...
  async function readFamilyProfilePhoto(file){
    ...
    const source=await new Promise((resolve,reject)=>{const reader=new FileReader();reader.onload=()=>resolve(String(reader.result||''));reader.onerror=()=>reject(new Error('No pude leer la foto'));reader.readAsDataURL(file)});
    const image=await new Promise((resolve,reject)=>{const node=new Image();node.onload=()=>resolve(node);node.onerror=()=>reject(new Error('No pude procesar la foto'));node.src=source});
    const side=Math.min(image.naturalWidth,image.naturalHeight);const x=(image.naturalWidth-side)/2;const y=(image.naturalHeight-side)/2;const canvas=document.createElement('canvas');canvas.width=256;canvas.height=256;const context=canvas.getContext('2d');context.clearRect(0,0,256,256);context.save();context.beginPath();context.arc(128,128,124,0,Math.PI*2);context.clip();context.drawImage(image,x,y,side,side,0,0,256,256);context.restore();return canvas.toDataURL('image/png');
  }
  ...
  function renderFamily(){
    ...
    const profileForm=$('familyProfileForm');profileForm.hidden=!viewer;if(viewer){$('familyProfileName').value=viewer.display_name||'';$('familyProfileInitials').textContent=familyInitials(viewer.display_name);renderFamilyProfilePhoto(viewer.profile_photo||'');const color=viewer.marker_color||'FOREST';profileForm.querySelectorAll('[name="familyMarkerColor"]').forEach(input=>{input.checked=input.value===color});profileForm.querySelector('.family-profile-preview').style.background=familyMarkerColors[color]||familyMarkerColors.FOREST}void renderFamilyMap();
  ...
  async function saveFamilyProfile(event){event.preventDefault();const form=event.currentTarget;const button=form.querySelector('button[type="submit"]');button.disabled=true;try{const marker=form.querySelector('[name="familyMarkerColor"]:checked')?.value||'FOREST';await api('/v1/home-family/profile',{method:'PUT',body:JSON.stringify({display_name:$('familyProfileName').value.trim(),marker_color:marker,photo_data_url:familyProfilePhotoData})});await refreshFamily();announce('Tu nombre, foto y marcador de Nexo quedaron guardados')}catch(error){announce(error.message)}finally{button.disabled=false}}
  ...
  function bind(){
    ...
    $('familyProfileName').addEventListener('input',()=>{$('familyProfileInitials').textContent=familyInitials($('familyProfileName').value)});
    $('familyProfilePhoto').addEventListener('change',async event=>{try{const file=event.currentTarget.files?.[0];if(!file)return;renderFamilyProfilePhoto(await readFamilyProfilePhoto(file));announce('Foto preparada. Pulsa Guardar identidad para aplicarla.')}catch(error){announce(error.message);event.currentTarget.value=''}});
