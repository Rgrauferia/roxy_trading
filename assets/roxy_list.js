(() => {
  ...
  function adaptPetQuestions(pet={}){const species=$('petProfileSpecies').value||pet.species||'other';const options=homeFood.pet_options||{};fillPetDataList('petBreedOptions',(options.breeds||{})[species]||[]);fillPetDataList('petExactSpeciesOptions',(options.exact_species||{})[species]||[]);$('petBreedLabel').textContent=species==='dog'||species==='cat'?'Raza exacta':'Raza o variedad';$('petExactSpeciesLabel').textContent=['fish','reptile','amphibian','bird'].includes(species)?'Especie exacta':'Tipo o especie exacta';renderPetChoices('petAllergyChoices',(options.allergies||{})[species]||(options.allergies||{}).default,pet.allergies);renderPetChoices('petConditionChoices',(options.conditions||{})[species]||(options.conditions||{}).default,pet.conditions);renderPetChoices('petGoalChoices',(options.goals||{})[species]||(options.goals||{}).default,pet.goals);adaptPetEnvironment()}
  ...
  function validatePetStep(){if(petProfileStep===1){if(!$('petProfileSpecies').value)return'Selecciona el tipo de mascota.';if(!$('petProfileName').value.trim())return'Escribe el nombre de tu mascota.'}return''}
