(() => {
  ...
  function renderPetDailySummary(pet,plan){
    ...
    const heading=document.createElement('div');const eyebrow=document.createElement('small');eyebrow.textContent='Hoy con '+pet.name;const title=document.createElement('strong');title.textContent=completed===daily.length&&daily.length?'Todo listo por hoy':daily.length?`${daily.length-completed} ${daily.length-completed===1?'cuidado pendiente':'cuidados pendientes'}`:'Plan listo';heading.append(eyebrow,title);identity.append(avatar,heading);
    const progress=document.createElement('span');progress.className='pet-today-progress';progress.textContent=daily.length?`${completed} de ${daily.length}`:'Listo';hero.append(identity,progress);root.append(hero);
    if(next){const nextCard=document.createElement('article');nextCard.className='pet-next-routine';const icon=document.createElement('span');icon.className='material-symbols-rounded';icon.setAttribute('aria-hidden','true');icon.textContent=next.icon||'event_repeat';const copy=document.createElement('div');const label=document.createElement('small');label.textContent=(next.cadence==='weekly'?'Esta semana':'Ahora sigue')+(petRoutineTimeLabel(next.time)?' · '+petRoutineTimeLabel(next.time):'');const nextTitle=document.createElement('strong');nextTitle.textContent=next.title;const detail=document.createElement('p');detail.textContent=next.detail;copy.append(label,nextTitle,detail);nextCard.append(icon,copy);if(!next.completed_today){const action=makeButton('Registrar','pet-next-action',event=>completePetRoutine(pet,next,event.currentTarget));nextCard.append(action)}root.append(nextCard)}
    const routineDetails=document.createElement('details');routineDetails.className='pet-routine-details';const summary=document.createElement('summary');summary.innerHTML='<span><strong>Rutina completa</strong><small>Abre solo cuando quieras revisar todos los cuidados</small></span><span class="material-symbols-rounded" aria-hidden="true">expand_more</span>';routineDetails.append(summary);const list=document.createElement('div');list.className='pet-routine-list';routines.forEach(routine=>{const row=document.createElement('article');row.className='pet-routine-row'+(routine.completed_today?' completed':'');const time=document.createElement('time');time.textContent=petRoutineTimeLabel(routine.time);const copy=document.createElement('div');const title=document.createElement('strong');title.textContent=routine.title;const meta=document.createElement('small');meta.textContent=routine.cadence==='weekly'?'Semanal':routine.detail;copy.append(title,meta);const button=document.createElement('button');button.type='button';button.className=routine.completed_today?'secondary':'primary';button.disabled=routine.completed_today;button.textContent=routine.completed_today?'Hecho ✓':'Registrar';button.addEventListener('click',()=>completePetRoutine(pet,routine,button));row.append(time,copy,button);list.append(row)});routineDetails.append(list);root.append(routineDetails);
  ...
  function renderPetCare(pet){const plan=(homeFood.pet_care_plans||{})[String(pet.id)]||{};$('petCareTitle').textContent='Hoy con '+pet.name;$('petCareIntro').textContent='Lo importante primero; el resto queda guardado para cuando quieras consultarlo.';renderPetDailySummary(pet,plan);const root=$('petCarePlan');root.replaceChildren();const guide=document.createElement('details');guide.className='pet-care-guide';const summary=document.createElement('summary');summary.innerHTML=`<span><strong>Guía personal de ${escapeHtml(pet.name)}</strong><small>Alimentación, hábitat, rutina, convivencia y señales a vigilar</small></span><span class="material-symbols-rounded" aria-hidden="true">expand_more</span>`;guide.append(summary);const content=document.createElement('div');content.className='pet-care-guide-content';(plan.sections||[]).forEach(section=>{const card=document.createElement('article');card.className='pet-care-card'+(section.urgent?' urgent':'')+(section.protected?' protected':'');const icon=document.createElement('span');icon.className='material-symbols-rounded';icon.setAttribute('aria-hidden','true');icon.textContent=section.icon||'pets';const copy=document.createElement('div');const title=document.createElement('h4');title.textContent=section.title;const text=document.createElement('p');text.textContent=section.text;copy.append(title,text);card.append(icon,copy);content.append(card)});if(plan.needs_exact_species){const notice=document.createElement('button');notice.type='button';notice.className='pet-care-exact';notice.innerHTML='<span class="material-symbols-rounded" aria-hidden="true">edit</span><span><strong>Falta la especie exacta</strong><small>Añádela para afinar parámetros, alimentación y convivencia.</small></span>';notice.addEventListener('click',()=>openPetProfile(pet));content.prepend(notice)}if(plan.source_url){const source=document.createElement('a');source.className='pet-care-source';source.href=plan.source_url;source.target='_blank';source.rel='noopener noreferrer';source.textContent='Fuente de cuidado: '+plan.source_label;content.append(source)}if(plan.legal_note){const legal=document.createElement('p');legal.className='pet-care-legal';legal.textContent=plan.legal_note;content.append(legal)}guide.append(content);root.append(guide)}
