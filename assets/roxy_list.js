(() => {
  ...
  async function hydrateRecipeImage(image,recipe,host,{hideOnMissing=false}={}){
    ...
    if(url.startsWith('data:image/')){image.src=url;return}
  ...
  function renderWeather(){
    ...
    apply('today');apply('calendar',true);
  }
  ...
  function renderRecipes() {
    ...
    const saved=[];const seenSavedTitles=new Set();(homeFood.recipes||[]).slice().reverse().forEach(recipe=>{const title=normalize(recipe.title||'');if(!title||seenSavedTitles.has(title))return;seenSavedTitles.add(title);saved.push(recipe)});const savedTitles=new Set(saved.map(recipe=>normalize(recipe.title||'')));const included=(homeFood.local_recipes||[]).filter(recipe=>!savedTitles.has(normalize(recipe.title||'')));const allRows=recipeFilter==='favorite'?saved:[...saved,...included];
    const rows=allRows.filter(recipe=>{const matchesSearch=!recipeSearch||normalize(`${recipe.title||''} ${recipe.subcategory||''}`).includes(recipeSearch);const matchesCategory=recipeFilter==='favorite'?recipe.favorite:recipeSearch||recipeCategoryId(recipe)===recipeFilter;return matchesSearch&&matchesCategory});
    $('recipeCount').textContent=recipeSearch||recipeFilter==='favorite'?`${rows.length} ${rows.length===1?'resultado':'resultados'}`:`${allRows.length} recetas disponibles`;
  ...
  function renderDesign(){
    ...
    projects.forEach(project=>{
      ...
      if(project.proposal_url&&project.proposal_status!=='GENERATING'){
        ...
        const compare=document.createElement('label');compare.className='design-comparison-control';const beforeText=document.createElement('span');beforeText.textContent='Antes';const slider=document.createElement('input');slider.type='range';slider.min='0';slider.max='100';slider.value='50';slider.setAttribute('aria-label','Comparar habitación actual y propuesta de Roxy');const afterText=document.createElement('span');afterText.textContent='Después';slider.addEventListener('input',()=>{after.style.clipPath=`inset(0 ${100-Number(slider.value)}% 0 0)`});compare.append(beforeText,slider,afterText);visuals.append(compare);
      }
      ...
      const actions=document.createElement('div');actions.className='design-project-actions';const generate=makeButton(project.proposal_status==='FAILED'?'Intentar propuesta otra vez':project.proposal_url?'Actualizar esta opción':'Crear esta propuesta','primary',()=>generateDesignProposal(project.id,project.selected_tier));generate.disabled=project.proposal_status==='GENERATING'||!homeDesign.generation_configured;const buy=makeButton('Comparar productos reales','secondary',()=>prepareDesignPurchase(project,card));const remove=makeButton('Eliminar proyecto','danger-button',()=>deleteDesignProject(project));actions.append(generate,buy,remove);
      body.append(heading,tierTabs,analysis,fit,products,revision,actions);card.append(visuals,body);root.append(card);
    });
  }
  ...
  async function analyzeDesignProject(projectId){
    try{announce('Roxy está revisando la luz, distribución y oportunidades del espacio');await api(`/v1/home-design/${encodeURIComponent(user)}/projects/${encodeURIComponent(projectId)}/analysis`,{method:'POST',body:'{}'});await refreshDesignProjects();announce('Análisis del espacio actualizado')}catch(error){announce(error.message)}
  }
  ...
  function bind(){
    ...
    $('calendarWeatherAsk').addEventListener('click',()=>{$('roxyCommand').value='Roxy, ¿cómo estará el clima esta semana?';openRoxyVoice()});
