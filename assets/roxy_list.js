(() => {
  ...
  async function saveFamilyProfile(event){event.preventDefault();const form=event.currentTarget;const button=form.querySelector('button[type="submit"]');button.disabled=true;try{const marker=form.querySelector('[name="familyMarkerColor"]:checked')?.value||'FOREST';const emoji=form.querySelector('[name="familyProfileEmoji"]:checked')?.value||'';await api('/v1/home-family/profile',{method:'PUT',body:JSON.stringify({display_name:$('familyProfileName').value.trim(),marker_color:marker,photo_data_url:familyProfilePhotoData,profile_emoji:emoji})});await refreshFamily();announce('Tu nombre, imagen y marcador de Nexo quedaron guardados')}catch(error){announce(error.message)}finally{button.disabled=false}}
  ...
  function bind(){
    ...
    $('familyProfilePhoto').addEventListener('change',async event=>{try{const file=event.currentTarget.files?.[0];if(!file)return;renderFamilyProfilePhoto(await readFamilyProfilePhoto(file));familyProfileEmoji='';$('familyProfileForm').querySelectorAll('[name="familyProfileEmoji"]').forEach(input=>{input.checked=input.value===''});announce('Foto preparada. Pulsa Guardar identidad para aplicarla.')}catch(error){announce(error.message);event.currentTarget.value=''}});
