(() => {
  ...
  function renderPlants(){
    ...
    if(!due.length&&!conditionPlants.length){const empty=document.createElement('div');empty.className='plant-priority-empty';empty.innerHTML=plants.length?'<span class="material-symbols-rounded" aria-hidden="true">task_alt</span><div><strong>Sin revisiones programadas hoy</strong><p>El calendario no confirma la salud de la planta. Observa la tierra y las hojas si notas un cambio.</p></div>':'<span class="material-symbols-rounded" aria-hidden="true">photo_camera</span><div><strong>Empieza con una foto</strong><p>Añade una planta para identificarla y crear su plan preventivo.</p></div>';tasksRoot.append(empty)}
  ...
  function openPlantDetail(plant){currentPlant=plant;$('plantDetailTitle').textContent=plant.display_name;const root=$('plantDetailContent');root.replaceChildren();const hero=document.createElement('div');hero.className='plant-detail-hero';const image=document.createElement('img');image.src=plant.photo_url;image.alt=`Foto de ${plant.display_name}`;const copy=document.createElement('span');copy.innerHTML=`<strong>${escapeHtml(plant.common_name||'Especie por confirmar')}</strong><em>${escapeHtml(plant.scientific_name||'')}</em><small>${escapeHtml(plant.room||'Sin ubicación')} · ${plant.placement==='outdoor'?'Exterior':'Interior'}</small>`;hero.append(image,copy);root.append(hero);
    ...
    const actions=document.createElement('div');actions.className='plant-detail-actions';actions.append(makeButton('Revisar con foto o video','primary',()=>addPlantJournal(plant)),makeButton('Editar condiciones','secondary',()=>openPlantForm(plant)),makeButton('Preparar compra de cuidado','secondary',()=>addPlantProduct(plant)),makeButton('Eliminar planta','danger-button',()=>deletePlant(plant)));root.append(actions);if(!$('plantDetailDialog').open)$('plantDetailDialog').showModal()}
