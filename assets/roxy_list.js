(() => {
  ...
  async function hydrateRecipeImage(image,recipe,host,{hideOnMissing=false}={}){
    ...
    image.classList.remove('recipe-image-loading');if(hideOnMissing)image.hidden=true;else{image.remove();host&&host.classList.add('no-photo')}
  }
  ...
  function recipeCard(recipe){
    ...
    const petLabel=recipe.audience==='pet'?({treat:'Premio ocasional',complement:'Complemento',veterinary_plan:'Plan veterinario'}[recipe.safety_class]||'Receta para mascota'):'';
  ...
  function renderRecipes() {
    ...
    const audienceRows=allRows.filter(recipe=>{if(recipeAudience!=='pet')return recipe.audience!=='pet';if(recipe.audience!=='pet'||(recipe.pet_species&&recipe.pet_species!==petSpecies))return false;const ingredients=normalize((recipe.ingredients||[]).map(row=>typeof row==='string'?row:row.name||'').join(' '));return !blockedIngredients.some(item=>ingredients.includes(item))});
    ...
    if(recipeAudience==='pet'){
      ...
      const heading=document.createElement('div');heading.className='recipe-category-heading';const copy=document.createElement('div');const title=document.createElement('h3');title.textContent=`Recetario de ${pet?.name||'tu mascota'}`;const description=document.createElement('p');description.textContent=blockedIngredients.length?`Filtrado según su especie y ${blockedIngredients.length} ${blockedIngredients.length===1?'restricción':'restricciones'} registradas`:'Premios y complementos separados de su alimento completo';copy.append(title,description);const count=document.createElement('span');count.textContent=String(rows.length);heading.append(copy,count);section.append(heading);
  ...
  function bind(){
    ...
    document.querySelectorAll('[data-pet-hub-tab]').forEach(button=>button.addEventListener('click',()=>{petHubTab=button.dataset.petHubTab;renderRecipes()}));
