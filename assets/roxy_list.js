(() => {
  ...
  function startSynchronizedStepVideo(){const media=currentStepVideo();if(!media)return;media.muted=true;media.loop=true;media.currentTime=0;media.dataset.syncActive='true';if(media.readyState>=2)media.play().catch(()=>{});else media.addEventListener('canplay',()=>{if(media.dataset.syncActive==='true')media.play().catch(()=>{})},{once:true})}
  function stopSynchronizedStepVideo(){const media=currentStepVideo();if(!media)return;delete media.dataset.syncActive;media.loop=false;media.pause()}
