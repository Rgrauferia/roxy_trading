...
# Design QA: Mobile Visual Dashboard Restoration (2026-07-20)

Source visual truth path: `/tmp/codex-remote-attachments/019f76dc-7cec-7ca2-a72e-9dcc2b978a54/090ABABB-F970-4ACC-9C74-B3F50125D32E/3-Photo-3.jpg` (Photo 3; Photo 2 is a supporting earlier concept).

Implementation screenshot path: unavailable because neither the in-app browser webview nor the Chrome extension could attach to the local Streamlit preview in this run.

Viewport: intended mobile QA viewport `390 × 844`.

State: authenticated Dashboard, AAPL, stock market, 1h.

**Full-view comparison evidence**

- The user-provided current screenshot shows the main `<section class="roxy-opening-stage ...">` emitted as a Markdown code block, followed by oversized single-column folder cards.
- The reference shows one compact visual system: top bar, Roxy centered between information rails, five compact folders in one row, chart and summary panels below.
- Source inspection identified the cause: the composed Dashboard string was passed directly to Markdown with leading indentation. The implementation now normalizes the complete fragment with `roxy_clean_html_fragment(...)` before rendering it.

**Focused region comparison evidence**

- Main stage: blocked pending a browser-rendered post-fix capture.
- Folder row: blocked pending a browser-rendered post-fix capture.
- Chart/summary region: blocked pending a browser-rendered post-fix capture.

**Findings**

- [P0 fixed in code] Dashboard markup displayed as text instead of UI.
  - Fix: clean and dedent the complete composed fragment before `st.markdown(..., unsafe_allow_html=True)`.
  - Verification: compilation and focused rendering-contract tests pass; browser evidence remains unavailable.
- [P1 pending visual confirmation] Mobile folder density and one-screen composition must be checked against Photo 3 after deployment.
  - Blocker: browser preview surfaces could not attach in this run.

**Primary interactions tested**

- Static route contracts remain present for opportunities, alerts, Roxy, education, diagnostics, analysis, calendar and portfolio.
- Browser interaction testing is blocked until a browser surface attaches or the user supplies the deployed post-fix screenshot.

**Console errors checked**

- Blocked: no browser-rendered session was available.

**Comparison history**

1. Earlier evidence: raw HTML visible and folders expanded vertically (Photo 1).
2. Fix made: normalize the complete Dashboard fragment before Markdown rendering; add a regression test that enforces this boundary.
3. Post-fix visual evidence: pending deployed mobile screenshot.

**Implementation checklist**

- [x] Stop HTML from leaking as visible text.
- [x] Preserve the existing Roxy stage, folder row and source-backed summary markup.
- [x] Add a rendering regression test.
- [ ] Capture the deployed authenticated Dashboard at the matching mobile viewport.
- [ ] Compare the main stage, folder row and chart/summary regions against Photo 3.
