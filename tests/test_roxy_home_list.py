...
def test_roxy_home_list_pwa_shell_is_installable_and_offline_capable():
    ...
    assert 'name="roxy-home-version" content="184"' in page.text
    ...
    assert 'href="/assets/roxy_list.css?v=129"' in page.text
    assert 'src="/assets/roxy_list.js?v=186"' in page.text
    assert '/assets/vendor/maplibre-gl.css?v=1' in worker.text
    assert '/assets/vendor/maplibre-gl.js?v=1' in worker.text
    assert '/assets/roxy_list.css?v=129' in worker.text
    assert '/assets/roxy_list.js?v=186' in worker.text
    for asset in ('roxy_home_weather_renderer.js?v=1', 'roxy_home_open_recipes.js?v=3', 'roxy_home_recipe_provider.js?v=2'):
        assert '/assets/' + asset in worker.text
        assert '/assets/' + asset in page.text
        assert client.get('/assets/' + asset).status_code == 200
    assert 'roxy-list-shell-v183' in worker.text
    ...
    assert "const APP_VERSION = '184'" in script.text
    ...
    assert '/v1/home-family/members/${encodeURIComponent(member.id)}/history?limit=1000' in script.text
