...
def test_roxy_home_list_pwa_shell_is_installable_and_offline_capable():
    ...
    assert 'name="roxy-home-version" content="217"' in page.text
    ...
    assert 'href="/assets/roxy_list.css?v=143"' in page.text
    assert 'src="/assets/roxy_list.js?v=225"' in page.text
    assert '/assets/vendor/maplibre-gl.css?v=1' in worker.text
    assert '/assets/vendor/maplibre-gl.js?v=1' in worker.text
    assert '/assets/roxy_list.css?v=143' in worker.text
    assert '/assets/roxy_list.js?v=225' in worker.text
    ...
    assert 'roxy-list-shell-v217' in worker.text
    for asset in ('roxy_home_living.js?v=7', 'roxy_home_living.css?v=5', 'roxy_fitness_planner.js?v=11', 'roxy_fitness_planner.css?v=2', 'roxy_fitness_training.js?v=3', 'roxy_fitness_training.css?v=4', 'roxy_fitness.js?v=16', 'roxy_fitness_progress.js?v=1', 'roxy_fitness_progress.css?v=1', 'roxy_home_more.css?v=4'):
        assert '/assets/' + asset in page.text and '/assets/' + asset in worker.text
        assert client.get('/assets/' + asset).status_code == 200
    assert 'COPY assets/roxy_fitness_progress.js assets/roxy_fitness_progress.css ./assets/' in Path('Dockerfile.roxy-home').read_text()
    assert 'COPY assets/roxy_fitness_training.js assets/roxy_fitness_training.css ./assets/' in Path('Dockerfile.roxy-home').read_text()
    for image in ('living-room-205.png', 'kitchen-room-205.png', 'garden-room-206.png', 'fitness-room-206.png', 'pets-room-206.png'):
        assert client.get('/assets/roxy_home/world/' + image).content.startswith(b'\x89PNG')
    assert 'COPY assets/roxy_home_living.js assets/roxy_home_living.css ./assets/' in Path('Dockerfile.roxy-home').read_text()
    assert 'COPY assets/roxy_home/world ./assets/roxy_home/world' in Path('Dockerfile.roxy-home').read_text()
    ...
    assert 'COPY assets/roxy_recipe_guide.js assets/roxy_recipe_guide.css ./assets/' in Path('Dockerfile.roxy-home').read_text()
    for asset in ('roxy_recipe_onboarding.js?v=3', 'roxy_recipe_onboarding.css?v=1'):
        assert '/assets/' + asset in page.text and '/assets/' + asset in worker.text
        assert client.get('/assets/' + asset).status_code == 200
    assert page.text.index('roxy_recipe_onboarding.js') < page.text.index('roxy_list.js')
    for asset in ('roxy_home_recovery.js?v=1', 'roxy_home_recovery.css?v=1'):
        assert '/assets/' + asset in page.text and '/assets/' + asset in worker.text
        assert client.get('/assets/' + asset).status_code == 200
    assert page.text.index('roxy_home_recovery.js') < page.text.index('roxy_home_registration.js')
    ...
    for asset in ('roxy_home_drinks.js?v=7', 'roxy_home_drinks.css?v=5'):
        assert '/assets/' + asset in page.text and '/assets/' + asset in worker.text
        assert client.get('/assets/' + asset).status_code == 200
    ...
    assert 'RoxyMyPlateRecipes?.setActive' in script.text
    assert 'COPY assets/roxy_home_map_readiness.js ./assets/roxy_home_map_readiness.js' in Path('Dockerfile.roxy-home').read_text()
    ...
    nav = page.text.split('<nav class="bottom-nav"', 1)[1].split('</nav>', 1)[0]
    assert nav.count('<button ') == 3
    assert 'data-tab-link="house"' in nav and 'data-tab-link="more"' in nav
    assert 'id="roxyVoiceLauncher"' in nav and '>Espacios<' in nav
    spaces = page.text.split('<nav class="more-rooms"', 1)[1].split('</nav>', 1)[0]
    for destination in ('recipes', 'fitness', 'shopping', 'pets', 'plants', 'design', 'family', 'calendar'):
        assert spaces.count(f'data-tab-link="{destination}"') == 1
    assert 'id="designProjectForm"' in page.text
    assert '/v1/home-design/' in script.text
    assert 'Revisar productos' in script.text
    assert "const APP_VERSION = '217'" in script.text
    ...
    assert '/assets/roxy_home_registration.js?v=4' in page.text
    assert '/assets/roxy_home_registration.js?v=4' in worker.text
    assert 'COPY assets/roxy_home_registration.js ./assets/roxy_home_registration.js' in Path('Dockerfile.roxy-home').read_text()
    assert 'https://challenges.cloudflare.com' in page.headers['content-security-policy']
    ...
    assert '/v1/home-family/members/${encodeURIComponent(memberId)}/history?limit=1000' in script.text
    assert 'familyHistoryGeneration===request.generation' in script.text
    assert 'clip-path:circle(50%)' in style.text
    ...
    assert '/assets/roxy_garden_guide.js?v=4' in page.text
    ...
    assert 'id="homeNavigation"' in page.text
    assert "family-map-person" in script.text
    assert "createFamilyMapPlace" in script.text
    assert "family-map-person-live" in script.text
    assert "family-map-place-badge" in style.text
    assert "family-map-place.active .family-map-place-label" in style.text
    assert "familyRouteMode" in script.text
    ...
    assert "Por qué aparece para " in script.text
    assert "Antes de elegir para " in script.text
    ...
    assert "cantidades para preparar, no ración diaria" in script.text
    assert "piezas preparadas" not in script.text
    ...
    assert "mealPlanForm').requestSubmit()" not in script.text
    assert "$('mealPlanForm').addEventListener('submit',createWeeklyPlan)" in script.text
...
def test_roxy_home_nexo_weather_transitions_automatically_to_real_radar_globe():
    ...
    assert 'id="familyWeatherGlobePanel"' in page
    ...
    assert "Number(familyMap.getZoom())<=5" in script
    assert "activateFamilyWeatherGlobe()" in script
    assert "exitFamilyWeatherGlobe" in script
    ...
    assert "No se muestran datos de radar simulados." in script
    assert "Radar real de precipitación" in page
    assert "Ambiente visual según Open-Meteo" in page
    assert "mediciones reales de radar de RainViewer" in page
    assert ".family-weather-globe-panel" in style
