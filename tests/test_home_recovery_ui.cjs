...
test('a superseded management status response cannot populate another member dialog',async()=>{
  const pending=deferred(),h=harness({fetch:async(_path,_options,n)=>n===1?pending.promise:response({enabled:false,remaining:0})});h.api.openManage({id:'member-a'});await flush();h.api.openManage({id:'member-b'});await flush();pending.resolve(response({enabled:true,remaining:8}));await flush();assert.equal(h.requests[0].options.signal.aborted,true);assert.match(h.document.body.textContent,/No tienes códigos de recuperación vigentes/);assert.equal(h.document.body.textContent.includes('8 códigos de recuperación disponibles'),false);
});
