...
assert.equal(all(panel, 'ol').reduce((count, list) => count + list.children.length, 0), 36);
...
assert.equal(sourceImages.length, 3, 'only the three photographs embedded by selected source revisions');
...
assert.deepEqual(photographed.map(row => row.id), ['wikibooks-385022', 'wikibooks-150880', 'wikibooks-219286']);
photographed.forEach(row => {
  ...
  assert.equal(row.photo_scope, 'Fotografía publicada con la receta original; no verifica cantidades ni resultado');
...
assert.equal(sourceImages[0].referrerPolicy, 'no-referrer'); assert.equal(sourceImages[0].width, 1024);
...
assert.ok(cuisine.children.some(option => option.textContent === 'España'));
search.value = 'saffron'; await search.emit('input');
assert.equal(cards(panel).length, 1); assert.equal(cards(panel)[0].firstChild.textContent, 'Risotto II');
search.value = ''; cuisine.value = 'Spanish'; await cuisine.emit('change');
assert.equal(cards(panel).length, 1); assert.equal(cards(panel)[0].firstChild.textContent, 'Spanish Potato Omelette');
...
assert.equal(reader.hidden, false); assert.equal(reader.children[0].textContent, 'Paso 1 de 8');
...
assert.equal(reader.children[0].textContent, 'Paso 8 de 8'); assert.equal(findButton(card, 'Siguiente').disabled, true);
await findButton(card, 'Anterior').click(); assert.equal(reader.children[0].textContent, 'Paso 7 de 8');
...
assert.equal(downloads[0].filename, 'roxy-original-wikibooks-385022.txt');
...
assert.equal(cards(stalePanel).length, 1); assert.equal(cards(stalePanel)[0].firstChild.textContent, 'Risotto II', 'stale household response must be discarded');
