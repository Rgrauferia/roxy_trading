...
def test_pending_photo_is_not_treated_as_an_image_blob(statuses, expected_fetches, removed):
    ...
    const image={classList:{add(){},remove(){}},addEventListener(name,fn){imageEvents.set(name,fn)},removeEventListener(name,fn){if(imageEvents.get(name)===fn)imageEvents.delete(name)},remove(){removed=true},set src(v){src=v;imageEvents.get('load')?.()}};
    const messages=[];
    ...
    const setTimeout=(callback,delay)=>{if(delay===3000)callback();return 1};
    const clearTimeout=()=>{};
...
def test_lazy_grid_keeps_all_new_cards_until_the_render_is_attached():
    ...
    const image=()=>{const events=new Map();return {isConnected:false,loading:'lazy',classList:{add(){},remove(){}},addEventListener(name,fn){events.set(name,fn)},removeEventListener(name,fn){if(events.get(name)===fn)events.delete(name)},get src(){return this._src},set src(value){this._src=value;events.get('load')?.()}}};
    """ + functions + r"""
